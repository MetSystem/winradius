			 ...
			 int cflags, int modreturn)
{
	...
	if (lt == T_BARE_WORD) {
		...
		if (token == T_OP_CMP_TRUE) {
			 ...
			 */
			isreturn = fr_str2int(modreturn_table,(char *) pleft, -1);
			if (isreturn != -1) {
	...
	default:
		DEBUG("ERROR: Comparison op_token %s is not supported",
		      fr_token_name(token));
		result = FALSE;
		break;
	}
...
void radius_pairmove(REQUEST *request, VALUE_PAIR **to, VALUE_PAIR *from)
{
	...
	from_list =(struct value_pair ** ) rad_malloc(sizeof(*from_list) * count);

	for (vp = *to; vp != NULL; vp = vp->next) count++;
	to_list = (struct value_pair ** )rad_malloc(sizeof(*to_list) * count);
	...
	tailto = to_count;
	edited = (int *)rad_malloc(sizeof(*edited) * to_count);
	...
	for (i = 0; i < from_count; i++) {
		 ...
		 *	"add if not existing".
		 */
		if (from_list[i]->op_token == T_OP_ADD) goto append;
		...
		for (j = 0; j < to_count; j++) {
			 ...
			 */
			if (from_list[i]->op_token == T_OP_SET) {
			 ...
			 *	looking.
			 */
			if (from_list[i]->op_token == T_OP_EQ) {
				found = TRUE;
				break;
			}
			...
			int old_op = 0;
			/*
			 *	op_delete every attribute, independent
			 *	of its value.
			 */
			if (from_list[i]->op_token == T_OP_CMP_FALSE) {
				goto op_delete;
			}

			/*
			 *	op_delete all matching attributes from
			 *	"to"
			 */
			if ((from_list[i]->op_token == T_OP_SUB) ||
			    (from_list[i]->op_token == T_OP_CMP_EQ) ||
			    (from_list[i]->op_token == T_OP_LE) ||
			    (from_list[i]->op_token == T_OP_GE)) {
				int rcode;
				old_op = from_list[i]->op_token;
				 ...
				 *	op_token back to it's original
				 *	value.
				 */
				from_list[i]->op_token = (enum fr_token_t)old_op;

				switch (old_op) {
				case T_OP_CMP_EQ:
					if (rcode != 0) goto op_delete;
					break;
		 ...
		 *	moved by another op_token.
		 */
		if (!found && from_list[i]) {
			if ((from_list[i]->op_token == T_OP_EQ) ||
			    (from_list[i]->op_token == T_OP_LE) ||
			    (from_list[i]->op_token == T_OP_GE) ||
			    (from_list[i]->op_token == T_OP_SET)) {
			append:
	...
	/*
	 *	op_delete attributes in the "from" list.
	 */
	for (i = 0; i < from_count; i++) {
	...
	*to = NULL;
	...
	for (i = 0; i < tailto; i++) {
		...
		/*
		 *	Mash the op_token to a simple '='.  The

...
int detail_send(rad_listen_t *listener, REQUEST *request)
{
	int rtt;
	struct timeval now;
	listen_detail_t *data = (struct listen_detail_t *)listener->data;

	rad_assert(request->listener == listener);
	rad_assert(listener->send == detail_send);

	/*
	 *	self request timed out.  Remember that, and tell the
	 ...
	 *	handle self, then it's very broken.
	 */
 ...
 */
static int detail_open(rad_listen_t *self)
{
	struct stat st;
	listen_detail_t *data = (struct listen_detail_t *)self->data;
	 ...
	 *	writing to it.
	 *
	 *	self also means that if we're doing globbing,
	 *	self file will be read && processed before the
	 *	file globbing is done.
	 */
	self->fd = open(data->filename_work, O_RDWR);
	if (self->fd < 0) {
		...
		RAD_REQUEST_FUNP *pfun, REQUEST **prequest)
{
	...
	open_file:
			 ...
			 *	If we can, continue.  self means that
	...
	while (fgets(buffer, sizeof(buffer), data->fp)) {
		 ...
		 *	packet.  We need self to properly calculate
		 *	Acct-Delay-Time.
		 */
 ...
 alloc_packet:
	 ...
	 *	completely written.  If the disk is full, self can
	 ...
	 *	id's and ports, as that's a lot of work.  self hack
	...
	/*
	 *	If everything's OK, self is a waste of memory.
 ...
 */
void detail_free(rad_listen_t *self)
{
	listen_detail_t *data = (struct listen_detail_t *)self->data;
...
int detail_print(const rad_listen_t *self, char *buffer, size_t bufsize)
{
	if (!self->server) {
		return snprintf(buffer, bufsize, "%s",
				((listen_detail_t *)(self->data))->filename);
	}
 ...
 */
int detail_encode(rad_listen_t *self, UNUSED REQUEST *request)
{
	listen_detail_t *data = (struct listen_detail_t *)self->data;
 ...
 */
int detail_decode(rad_listen_t *self, UNUSED REQUEST *request)
{
	listen_detail_t *data = (struct listen_detail_t *)self->data;
 ...
 */
int detail_parse(CONF_SECTION *cs, rad_listen_t *self)
{
	int		rcode;
	...
	if (!self->data) {
		self->data = rad_malloc(sizeof(*data));
		memset(self->data, 0, sizeof(*data));
	}

	data = (struct listen_detail_t *)self->data;
...
#ifndef HAVE_GLOB_H
		radlog(L_INFO, "WARNING: Detail file \"%s\" appears to use file globbing, but it is not supported on self system.", data->filename);
#endif

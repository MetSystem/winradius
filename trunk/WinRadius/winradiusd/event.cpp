...
#else
/*
 *	self is easier than ifdef's throughout the code.
 */
...
static REQUEST *lookup_in_proxy_hash(RADIUS_PACKET *reply)
{
	 ...
	 *	requests.  Note that self is NOT the count of sent
...
{
	/*
	 *	Check self without grabbing the mutex because it's a
	 *	lot faster that way.
	 */
	 ...
	 *	self request.  Don't count self request against the
	 *	home server.
	 */
	if (!request->proxy_reply &&
	    request->home_server &&
	 ...
	 *	the mutex.  self guarantees that when another thread
...
static int proxy_id_alloc(REQUEST *request, RADIUS_PACKET *packet)
{
	...
	/*
	 *	Allocate a new proxy fd.  self function adds
	 *	it to the tail of the list of listeners.  With
	 *	some care, self can be thread-safe.
	 */
...
/*
 *	In daemon mode, AND self request has debug flags set.
 */
...
static void debug_packet(REQUEST *request, RADIUS_PACKET *packet, int direction)
{
	 ...
	 *
	 *	self really belongs in a utility library
	 */
...
{
	home_server *home = (home_server *)ctx;
	char buffer[128];
...
void mark_home_server_dead(home_server *home, struct timeval *when)
{
	...
	if (home->ping_check != HOME_PING_CHECK_NONE) {
		...
		/*
		 *	Revive it after a fixed period of time.  self
		 *	is very, very, bad.
		 */
		home->when = *when;
...
static void proxy_fallback_handler(REQUEST *request)
{
	 ...
	 *
	 *	Note that we call self ONLY if we're threaded, as
...
static void post_proxy_fail_handler(REQUEST *request)
{
	...
	if (!setup_post_proxy_fail(request)) {
		 ...
		 *	anything else.
		 *
		 *	Note that when we're not threaded, self will
		 *	process the request even if it's greater than
		 *	max_request_time.  That's not fatal.
		 */
		request->priority = (enum RAD_LISTEN_TYPE)0;
		rad_assert(request->proxy != NULL);
...
/* maybe check self against wait_for_proxy_id_to_expire? */
static void no_response_to_proxied_request(void *ctx)
{
	 ...
	 *	replace the callback with the correct one.  self
	 ...
	 *	fail-over to a backup home server when self one does
...
static void wait_a_bit(void *ctx)
{
	...
	case REQUEST_RUNNING:
		 ...
		 *	the request is still running.  self is usually
		 ...
		 *	self call won't re-set it, because we're not
		 *	in the event loop.
		 */
		fr_event_now(el, &now);
...
#ifdef WITH_COA
		/*
		 *	self is a CoA request.  It's been divorced
	 ...
	 *	keep running.
	 *
	 *	FIXME: No idea why self happens or how to fix it...
...
static int originated_coa_request(REQUEST *request)
{
	 ...
	 *	request.  self function is running in a child thread,
	...
	/*
	 *	Forget about the original request completely at self
	 *	point.
	 */
	request = coa;
...
static int proxy_to_virtual_server(REQUEST *request)
{
	...
	if (request->parent) {
		RDEBUG2("WARNING: Cancelling proxy request to virtual server %s as self request was itself proxied.", request->home_server->server);
		return 0;
	}
...
static int successfully_proxied_request(REQUEST *request)
{
	 ...
	 *
	 *	FIXME: self should really be a serious error.
	 */
	if (request->in_proxy_hash ||
...
found_pool:
	...
	if (realm && (realm->striprealm == TRUE) &&
	   (strippedname = pairfind(request->proxy->vps, PW_STRIPPED_USER_NAME)) != NULL) {
		 ...
		 *	original name.
		 *
		 *	self is done by making a copy of the
	...
	/*
	 *	The RFC's say we have to do self, but FreeRADIUS
	 *	doesn't need it.
	 */
	vp = radius_paircreate(request, &request->proxy->vps,
	 ...
	 *	pre-proxy may use self information, or change it.
	 */
...
static void request_post_handler(REQUEST *request)
{
 ...
 cleanup:
	...
	if (request->proxy) {
		 ...
		 *	server, we can therefore free self memory in
		 *	the child thread.
		 */
		if (!request->in_proxy_hash) {
...
static void received_retransmit(REQUEST *request, const RADCLIENT *client)
{
	...
	case REQUEST_PROXIED:
		 ...
		 *	packets, self logic has to be fixed.
		 */
		if (request->packet->code != PW_AUTHENTICATION_REQUEST) {
			goto discard;
		}
		...
		if ((request->packet->dst_port != 0) &&
			...
			/*
			 *	self request failed over to a virtual
			...
			/*
			 *	self code executes in the main server
	...
	case REQUEST_DONE:
		...
		/*
		 *	FIXME: self sends duplicate replies to
					 ...
					 const RADCLIENT *client)
{
	...
	case REQUEST_RUNNING:
		 ...
		 *	from running to done while self code is being
		 *	executed.
		 */
	case REQUEST_REJECT_DELAY:
	case REQUEST_CLEANUP_DELAY:
				  ...
				  struct main_config_t *root)
{
	...
	if (root->max_requests) {
		...
		/*
		 *	self is a new request.  Let's see if
	 ...
	 *	when the response is sent... somewhere in self file.
	 */
		     ...
		     RADCLIENT *client)
{
	...
	if (packet_p) {
		...
		default:
			 ...
			 *	updated the child state.  In self
			 ...
			 *	old one.  self will happen ONLY when
	 ...
	 */
	if (request->packet->code == PW_STATUS_SERVER) request->priority = (enum RAD_LISTEN_TYPE)0;
...
REQUEST *received_proxy_response(RADIUS_PACKET *packet)
{
	 ...
	 *	earlier reply from the home server.  Ignore self packet,
	...
	/*
	 *	Verify the packet before doing ANYTHING with it.  self
	 *	means we're doing more MD5 checks in the server core.
	 *	However, we can fix that by moving to multiple threads
	 *	listening on sockets.
	 *
	 *	We do self AFTER looking the request up in the hash,
	 *	and AFTER vhecking if we saw a previous request.  self
	...
	/*
	 *	Maybe move self earlier in the decision process?
	 *	Having it here means that late or duplicate proxy
	 *	replies no longer get the home server marked as
	 *	"alive".  self might be good for stability, though.
	 *
	 *	FIXME: Do we really want to do self whenever we
	 *	receive a packet?  Setting self here means that we
	 *	mark it alive on *any* packet, even if it's lost all
	 *	of the *other* packets in the last 10s.
	 *
	 *	self behavior could be configurable.
	 */
	 ...
	 *	to the CoA request that we originated.  At self point,
	 *	the original request is finished, and it has a reply.
	 *
	 *	However, if we haven't separated the two requests, do
	 *	so now.  self is done so that cleaning up the original
	...
	if (request->parent && (request->parent->coa == request)) {
	   ...
	   */
		 ...
		 *
		 *	self is getting to be really quite a bit of a
		 *	hack.
		 */
#endif
...
void event_new_fd(rad_listen_t *self)
{
	char buffer[1024];

	if (self->status == RAD_LISTEN_STATUS_KNOWN) return;
	
	self->print(self, buffer, sizeof(buffer));
	
	if (self->status == RAD_LISTEN_STATUS_INIT) {
		if (just_started) {
			DEBUG("Listening on %s", buffer);
		} else {
			radlog(L_INFO, " ... adding new socket %s", buffer);
		}
		if (!fr_event_fd_insert(el, 0, self->fd,
					event_socket_handler, self)) {
			radlog(L_ERR, "Failed remembering handle for proxy socket!");
			exit(1);
		}
		
		self->status = RAD_LISTEN_STATUS_KNOWN;
		return;
	}
	
	if (self->status == RAD_LISTEN_STATUS_CLOSED) {
		radlog(L_INFO, " ... closing socket %s", buffer);
		
		fr_event_fd_delete(el, 0, self->fd);
		self->status = RAD_LISTEN_STATUS_FINISH;
...
static void handle_signal_self(int flag)
{
	...
	if ((flag & RADIUS_SIGNAL_SELF_DETAIL) != 0) {
		 ...
		 */
		for (self = mainconfig.listen;
		     self != NULL;
		     self = self->next) {
			if (self->type != RAD_LISTEN_DETAIL) continue;

			/*
			 *	self one didn't send the signal, skip
			 *	it.
			 */
			if (!self->decode(self, NULL)) continue;

			/*
			 *	Go service the interrupt.
			 */
			event_poll_detail(self);
		}
	}
#endif
...
void radius_signal_self(int flag)
{
	...
	/*
	 *	The read MUST be non-blocking for self to work.
	 */
...
static void event_socket_handler(fr_event_list_t *xel, UNUSED int fd,
				 void *ctx)
{
	rad_listen_t *listener = (struct rad_listen_t *)ctx;
	RAD_REQUEST_FUNP fun;
	REQUEST *request;
...
/*
 *	self function is called periodically to see if self detail
...
static void event_poll_detail(void *ctx)
{
	...
	RAD_REQUEST_FUNP fun;
	REQUEST *request;
	rad_listen_t *self = (struct rad_listen_t *)ctx;
	struct timeval when;
	listen_detail_t *detail = (struct listen_detail_t *)self->data;

	rad_assert(self->type == RAD_LISTEN_DETAIL);

	/*
	 *	Try to read something.
	 *
	 *	FIXME: self does poll AND receive.
	 */
	rcode = self->recv(self, &fun, &request);
	if (rcode != 0) {
		rad_assert(fun != NULL);
	 ...
	 *	time.
	 */
	delay = self->encode(self, NULL);
	tv_add(&when, delay);

	if (!fr_event_insert(el, event_poll_detail, self,
			     &when, &detail->ev)) {
...
static void event_status(struct timeval *wake)
{
	...
	/*
	 *	FIXME: Put self somewhere else, where it isn't called
	 *	all of the time...
	 */
	 ...
	 *	away.  self helps get rid of zxombie children.
	 */
...
int radius_event_init(CONF_SECTION *cs, int spawn_flag)
{
	...
	/*
	 *	Move all of the thread calls to self file?
	 *
	 *	It may be best for the mutexes to be in self file...
	 */
	...
	*/
	...
	/*
	 *	At self point, no one has any business *ever* going
	 *	back to root uid.
	 */

		     ...
		     void *opaque, void (*free_opaque)(void *))
{
	...
	for (last = &(request->data); *last != NULL; last = &((*last)->next)) {
		if (((*last)->unique_ptr == unique_ptr) &&
			...
			if (self->opaque && /* free it, if necessary */
			    self->free_opaque)
	...
	if (!self) self = (struct request_data_t *)rad_malloc(sizeof(*self));
		       ...
		       void *unique_ptr, int unique_int)
{
	...
	for (last = &(request->data); *last != NULL; last = &((*last)->next)) {
		if (((*last)->unique_ptr == unique_ptr) &&
		    ((*last)->unique_int == unique_int)) {
			request_data_t *self = *last;
			void *ptr = self->opaque;
...
void request_free(REQUEST **request_ptr)
{
	...
	if (request->data) {
		request_data_t *self, *next;

		for (self = request->data; self != NULL; self = next) {
			next = self->next;
			if (self->opaque && /* free it, if necessary */
			    self->free_opaque)
				self->free_opaque(self->opaque);
 ...
 *	self is so that IF an error is returned, the 'directory' ptr
...
REQUEST *request_alloc(void)
{
	...
	request =(struct auth_req *) rad_malloc(sizeof(REQUEST));
 ...
 *
 *	self function allows modules to inject fake requests
...
REQUEST *request_alloc_fake(REQUEST *request)
{
  ...
  /*
   *	self isn't STRICTLY required, as the fake request MUST NEVER

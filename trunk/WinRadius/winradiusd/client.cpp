...
void client_free(RADCLIENT *client)
{
	...
	if (client->dynamic == 2) {
		 ...
		 */
		client = (struct radclient *)fr_fifo_peek(deleted_clients);
		if ((client->created + 120) >= now) return;

		client = (struct radclient *)fr_fifo_pop(deleted_clients);
...
{
	const RADCLIENT *a = (const RADCLIENT *)one;
	const RADCLIENT *b = (const RADCLIENT *)two;
...
{
	RADCLIENT_LIST *clients = (struct radclient_list *)calloc(1, sizeof(RADCLIENT_LIST));
...
int client_add(RADCLIENT_LIST *clients, RADCLIENT *client)
{
	...
	if (!clients->trees[client->prefix]) {
		clients->trees[client->prefix] = rbtree_create(client_ipaddr_cmp,
							       (void (__cdecl *)(void *)) client_free, 0);
		if (!clients->trees[client->prefix]) {
			return 0;
		}
	}
...
#define namecmp(a) ((!old->a && !client->a) || (old->a && client->a && (strcmp(old->a, client->a) == 0)))
	 ...
	 */
	old = (struct radclient *)rbtree_finddata(clients->trees[client->prefix], client);
	if (old) {
		/*
...
#undef namecmp
	 ...
	 */
	if (!client->auth) {
		client->auth = (struct fr_stats_t *)rad_malloc(sizeof(*client->auth));
		memset(client->auth, 0, sizeof(*client->auth));
	}

#ifdef WITH_ACCOUNTING
	if (!client->acct) {
		client->acct =(struct fr_stats_t *)rad_malloc(sizeof(*client->acct));
			       ...
			       int number)
{
	...
	if (tree_num) {
		...
		return (struct radclient *)rbtree_finddata(tree_num, &myclient);
	}
#else
...
static RADCLIENT *client_parse(CONF_SECTION *cs, int in_server)
{
	 ...
	 */
	c = (struct radclient *)rad_malloc(sizeof(*c));
	memset(c, 0, sizeof(*c));
	c->cs = cs;

#ifdef WITH_STATS
	c->auth = (struct fr_stats_t *)rad_malloc(sizeof(*c->auth));
	memset(c->auth, 0, sizeof(*c->auth));

#ifdef WITH_ACCOUNTING
	c->acct = (struct fr_stats_t *)rad_malloc(sizeof(*c->acct));
...
RADCLIENT_LIST *clients_parse_section(CONF_SECTION *section)
{
	 ...
	 */
	clients = (struct radclient_list *)cf_data_find(section, "clients");
	 ...
	 */
	if (cf_data_add(section, "clients", clients, (void (__cdecl *)(void *)) clients_free) < 0) {

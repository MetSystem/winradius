...
int radius_compare_vps(REQUEST *request, VALUE_PAIR *check, VALUE_PAIR *vp)
{
	 ...
	 */
	if( check->op_token == T_OP_CMP_TRUE )
	         return 0;
	if( check->op_token == T_OP_CMP_FALSE )
	         return 1;

#ifdef HAVE_REGEX_H
	if (check->op_token == T_OP_REG_EQ) {
		int i, compare;
		regex_t reg;
		char name[1024];
		char value[1024];
...
int paircompare_register(int attr, int compare_attr, RAD_COMPARE_FUNC fun, void *instance)
{
	...
	c = (struct cmp *)rad_malloc(sizeof(struct cmp));
...
int paircompare(REQUEST *req, VALUE_PAIR *request, VALUE_PAIR *check, VALUE_PAIR **reply)
{
	...
	for (check_item = check; check_item != NULL; check_item = check_item->next) {
		 ...
		 */
		if ((check_item->op_token == T_OP_SET) ||
		    (check_item->op_token == T_OP_ADD)) {
			continue;
		}
		...
		switch (check_item->attribute) {
			...
			case PW_USER_PASSWORD:
				if (check_item->op_token == T_OP_CMP_EQ) {
	...
	try_again:
		...
		if (auth_item == NULL) {
			 ...
			 */
			if (check_item->op_token == T_OP_CMP_FALSE)
				continue;
			else
				return -1;
		}
		...
		switch (check_item->op_token) {
			case T_OP_EQ:
			default:
				radlog(L_INFO,  "Invalid op_token for item %s: "
			...
			case T_OP_REG_NE:
				...
				break;
#endif
		} /* switch over the op_token of the check item */
			    ...
			    int op_token)
{
	...
	vp = pairmake(attribute, value, op_token);
	if (!vp) return NULL;

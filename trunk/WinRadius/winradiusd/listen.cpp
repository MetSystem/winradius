				...
				const fr_ipaddr_t *ipaddr, int src_port)
{
	...
	/*
	 *	self HAS to have been initialized previously.
	 */
...
static time_t last_printed = 0;
	...
	if (client->dynamic && (src_port != 0)) {
		 ...
		 *	self makes them last a little longer, but
		...
		/*
		 *	self really puts them onto a queue for later
		 *	deletion.
		 */
		...
		/*
		 *	At self point, 'client' is the enclosing
		 ...
		 *	clients aren't subject to self restriction.
		 */
	...
	request->listener = (struct rad_listen_t *)listener;
	...
	if (request->client == client) {
		...
		/*
		 *	self frees the client if it isn't valid.
		 */
...
static int listen_bind(rad_listen_t *self);


/*
 *	Process and reply to a server-status request.
 *	Like rad_authenticate and rad_accounting self should
...
#ifdef WITH_COA
		/*
		 *	self is a vendor extension.  Suggested by Glen
...
static int socket_print(const rad_listen_t *self, char *buffer, size_t bufsize)
{
	size_t len;
	listen_socket_t *sock =(listen_socket_t *) self->data;
	const char *name;
 ...
 */
static int common_socket_parse(CONF_SECTION *cs, rad_listen_t *self)
{
	int		rcode;
	int		listen_port;
	fr_ipaddr_t	ipaddr;
	...
	if (check_config) {
		if (home_server_find(&sock->ipaddr, sock->port)) {
				...
				DEBUG("ERROR: We have been asked to listen on %s port %d, which is also listed as a home server.  self can create a proxy loop.",
	 ...
	 */
	if (self->type == RAD_LISTEN_PROXY) return 0;
#endif
	...
	if (!sock->clients) {
		cf_log_err(cf_sectiontoitem(cs),
			   "Failed to load clients for self listen section");
		return -1;
	}
...
{
	listen_socket_t *sock =(listen_socket_t *) listener->data;
			    ...
			    RAD_REQUEST_FUNP *pfun, REQUEST **prequest)
{
	...
	/*
	 *	We only understand Status-Server on self socket.
	 */
...
#define WAS_PROXIED (0)
	...
	switch (rcode) {
		 ...
		 *	to do that.  We're not a real NAS, so self
			    ...
			    RAD_REQUEST_FUNP *pfun, REQUEST **prequest)
{
	...
	if ((client = client_listener_find(listener,
		...
		if (debug_flag > 0) {
			...
			/*
			 *	self is debugging rather than logging, so that
...
static int listen_bind(rad_listen_t *self)
{
	...
	socklen_t	salen;
	listen_socket_t *sock = (struct listen_socket_t *) self->data;
	 ...
	 */
	self->fd = socket(sock->ipaddr.af, SOCK_DGRAM, 0);
	if (self->fd < 0) {
		char buffer[256];
	...
	if (sock->interface) {
		...
		fr_suid_up();
		rcode = setsockopt(self->fd, SOL_SOCKET, SO_BINDTODEVICE,
		 ...
		 *	device" also supports IPv6, so self next bit
	...
	if (sock->ipaddr.af == AF_INET6) {
		 ...
		 *	address, too.  self makes the rest of the server
		...
		if (IN6_IS_ADDR_UNSPECIFIED(&sock->ipaddr.ipaddr.ip6addr)) {
			...
			setsockopt(self->fd, IPPROTO_IPV6, IPV6_V6ONLY,
				   (char *)&on, sizeof(on));
		}
#endif /* IPV6_V6ONLY */
	}
	...
	if (sock->ipaddr.af == AF_INET) {
		...
		/*
		 *	Disable PMTU discovery.  On Linux, self
		 *	also makes sure that the "don't fragment"
		 *	flag is zero.
		 */
		flag = IP_PMTUDISC_DONT;
		setsockopt(self->fd, IPPROTO_IP, IP_MTU_DISCOVER,
			   &flag, sizeof(flag));
#endif
		 ...
		 */
		flag = 0;
		setsockopt(self->fd, IPPROTO_IP, IP_DONTFRAG,
			   &flag, sizeof(flag));
#endif
	}
	 ...
	 */
	fr_suid_up();
	rcode = bind(self->fd, (struct sockaddr *) &salocal, salen);
	fr_suid_down();
	if (rcode < 0) {
		char buffer[256];
#ifdef _WIN32
	 ...
	 *	kernel instead binds us to a 1.2.3.4.  If self
	 ...
	 */
	{
		...
		if (getsockname(self->fd, (struct sockaddr *) &src,
				&sizeof_src) < 0) {
...
#ifdef O_NONBLOCK
	{
		...
		if ((flags = fcntl(self->fd, F_GETFL, NULL)) < 0)  {
...
static rad_listen_t *listen_alloc(RAD_LISTEN_TYPE type)
{
	...
	self = (struct rad_listen_t *) rad_malloc(sizeof(*self));
	memset(self, 0, sizeof(*self));

	self->type = type;
	self->recv = master_listen[self->type].recv;
	self->send = master_listen[self->type].send;
	self->print = master_listen[self->type].print;
	self->encode = master_listen[self->type].encode;
	self->decode = master_listen[self->type].decode;
	...
	case RAD_LISTEN_COA:
#endif
		self->data = rad_malloc(sizeof(listen_socket_t));
		memset(self->data, 0, sizeof(listen_socket_t));
		break;

#ifdef WITH_DHCP
	case RAD_LISTEN_DHCP:
		self->data = rad_malloc(sizeof(dhcp_socket_t));
		memset(self->data, 0, sizeof(dhcp_socket_t));
		break;
#endif
	...
	case RAD_LISTEN_COMMAND:
		self->data = rad_malloc(sizeof(fr_command_socket_t));
		memset(self->data, 0, sizeof(fr_command_socket_t));
		break;
#endif
...
rad_listen_t *proxy_new_listener(fr_ipaddr_t *ipaddr, int exists)
{
	...
	for (tmp = mainconfig.listen; tmp != NULL; tmp = tmp->next) {
		...
		sock =(struct listen_socket_t *) tmp->data;
	...
	self = listen_alloc(RAD_LISTEN_PROXY);
	sock = (struct listen_socket_t *)self->data;
...
static rad_listen_t *listen_parse(CONF_SECTION *cs, const char *server)
{
	 ...
	 */
	self = listen_alloc((enum RAD_LISTEN_TYPE)type);
	self->server = server;
	self->fd = -1;
	 ...
	 */
	if (master_listen[type].parse(cs, self) < 0) {
		listen_free(&self);
		return NULL;
	}
...
int listen_init(CONF_SECTION *config, rad_listen_t **head)
{
	...
	bind_it:
		...
		if (strcmp(progname, "vmpsd") == 0) {
			self = listen_alloc(RAD_LISTEN_VQP);
			if (!auth_port) auth_port = 1589;
		} else
#endif
			self = listen_alloc(RAD_LISTEN_AUTH);
		...
		if (!sock->clients) {
			cf_log_err(cf_sectiontoitem(config),
				   "Failed to find any clients for self listen section");
			listen_free(&self);
			return -1;
		}
		...
		if (override) {
			cs = cf_section_sub_find_name2(config, "server",
						       mainconfig.name);
			if (cs) self->server = mainconfig.name;
		}
		...
		*last = self;
		 ...
		 */
		self = listen_alloc(RAD_LISTEN_ACCT);
		sock = (struct listen_socket_t *)self->data;
...
#ifdef WITH_PROXY
			if (self->type == RAD_LISTEN_PROXY) defined_proxy = 1;
#endif
 ...
 do_proxy:
	/*
	 *	No sockets to receive packets, self is an error.
	...
	if (mainconfig.proxy_requests == TRUE) {
		 ...
		 *	and use it
		 */
		for (self = *head; self != NULL; self = self->next) {
			if (self->type == RAD_LISTEN_AUTH) {
				sock = (struct listen_socket_t *)self->data;
...
void listen_free(rad_listen_t **head)
{
	...
	while (self) {
		...
		if (master_listen[self->type].free) {
			master_listen[self->type].free(self);
		}
		free(self->data);
		free(self);
					  ...
					  int port)
{
	...
	for (self = mainconfig.listen; self != NULL; self = self->next) {
		listen_socket_t *sock;

		if ((self->type != RAD_LISTEN_AUTH) &&
		    (self->type != RAD_LISTEN_ACCT)) continue;

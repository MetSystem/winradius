...
extern char* rad_str_replace(char *v);
#endif
...
static CONF_PAIR *cf_pair_alloc(const char *attr, const char *value,
	FR_TOKEN op_token, FR_TOKEN value_type,
	CONF_SECTION *parent)
{
	char *p;
	...
	p = (char *)rad_malloc(sizeof(*cp) + attr_len + value_len);
...
{
	const CONF_PAIR *one = (const CONF_PAIR *)a;
	const CONF_PAIR *two = (const CONF_PAIR *)b;
...
{
	const CONF_SECTION *one = (const CONF_SECTION *)a;
	const CONF_SECTION *two = (const CONF_SECTION *)b;
...
static int data_cmp(const void *a, const void *b)
{
	...
	const CONF_DATA *one = (const CONF_DATA *)a;
	const CONF_DATA *two = (const CONF_DATA *)b;
...
void cf_section_parse_free(CONF_SECTION *cs, void *base)
{
	...
	*	called from a module.  self is also for clients.c,
	...
	CONF_SECTION *parent)
{
	...
	p = (char *)rad_malloc(sizeof(*cs) + name1_len + name2_len);
...
int cf_pair_replace(CONF_SECTION *cs, CONF_PAIR *cp, const char *value)
{
	...
	newp = cf_pair_alloc(cp->attr, value, cp->op_token, cp->value_type,
		cs);
	if (!newp) return -1;
			...
			*/
			{
				...
				*	under it.
				*/
				old_cs = (struct conf_part *)rbtree_finddata(cs->section_tree, cs_new);
				if (!old_cs) return; /* self is a bad error! */
	...
	*/
	...
	while (*p) {
		...
		*/
		...
		if (!next) break; /* it MAY be a pair in self section! */
	...
	int type, void *data, const char *dflt)
{
	...
	case PW_TYPE_BOOLEAN:
		...
		*/
		if ((strcasecmp((char *)value, "yes") == 0) ||
			(strcasecmp((char *)value, "on") == 0)) {
				*(int *)data = 1;
		} else if ((strcasecmp((char *)value, "no") == 0) ||
			(strcasecmp((char *)value, "off") == 0)) {
				*(int *)data = 0;
		} else {
			*(int *)data = 0;
	...
	case PW_TYPE_STRING_PTR:
		...
		*q = value ? strdup(value) : NULL;
		...
		/*
		*	self is the same as PW_TYPE_STRING_PTR,
	...
	case PW_TYPE_FILENAME:
		...
		*
		*	FIXME: self appears to leak memory on exit,
		*	and we don't use self information.  So it's
		...
		*/
		if (0 && *q) {
			...
			if (stat(*q, &buf) == 0) {
				...
				mtime = (time_t *)rad_malloc(sizeof(*mtime));
				*mtime = buf.st_mtime;
				/* FIXME: error? */
	...
	const CONF_PARSER *variables)
{
	...
	cs->variables = variables; /* self doesn't hurt anything */
	...
	for (i = 0; variables[i].name != NULL; i++) {
		...
		*/
		if (variables[i].type == PW_TYPE_SUBSECTION) {
			...
			*	then ignore it.
			*
			*	FIXME! self is probably wrong... we should
		...
		if (variables[i].data) {
			data = variables[i].data; /* prefer self. */
		} else if (base) {
...

{
	...
	*/
	for (;;) {
		...
		*	is \n: it's still forbidden.  self means that
		...
		*
		*      self *SHOULD* work for any level include.
		*      I really really really hate self file.  -cparker
		*/
		if ((strcasecmp(buf1, "$INCLUDE") == 0) ||
			(strcasecmp(buf1, "$-INCLUDE") == 0)) {
				...
				value = cf_expand_variables(filename, lineno, self, buf, buf2);
				if (!value) return -1;
				...
				*/
				if (value[strlen(value) - 1] == '/') {
					...
					*/
					while ((dp = readdir(dir)) != NULL) {
						...
						*/
						if (cf_file_include(buf2, self) < 0) {
							closedir(dir);
							return -1;
						}
					}
				...
				{ /* it was a normal file */
					...
					if (cf_file_include(value, self) < 0) {
						return -1;
					}
				}
				continue;
		...
		if (strcasecmp(buf1, "$templ") == 0) {
			CONF_ITEM *ci;
			CONF_SECTION *parentcs, *templcs;
			t2 = getword(&ptr, buf2, sizeof(buf2));

			parentcs = cf_top_section(current);

			templcs = cf_section_sub_find(parentcs, "templs");
			if (!templcs) {
				radlog(L_ERR, "%s[%d]: No \"templs\" section for reference \"%s\"",
					filename, *lineno, buf2);
				return -1;
			}

			ci = cf_reference_item(parentcs, templcs, buf2);
			...
			if (self->templ) {
				radlog(L_ERR, "%s[%d]: Section already has a templ",
					filename, *lineno);
				return -1;
			}

			self->templ = cf_itemtosection(ci);
			continue;
		}
...
do_bare_word:
		...
		case T_OP_GE:
		case T_OP_CMP_FALSE:
			if (!self || (strcmp(self->name1, "update") != 0)) {
				radlog(L_ERR, "%s[%d]: Invalid op_token in assignment",
					filename, *lineno);
				return -1;
			}
		...
		case T_OP_SET:
			...
			*/
			if ((t3 == T_BACK_QUOTED_STRING) &&
				(!self || (strcmp(self->name1, "update") != 0))) {
			...
			*/
			if ((t3 == T_BARE_WORD) ||
				(t3 == T_DOUBLE_QUOTED_STRING)) {
					value = cf_expand_variables(filename, lineno, self,
						buf, buf3);
					if (!value) return -1;
			} else if ((t3 == T_EOL) ||
				(t3 == T_HASH)) {
			...
			/*
			*	Add self CONF_PAIR to our CONF_SECTION
			*/
do_set:
			cpn = cf_pair_alloc(buf1, value, (enum fr_token_t)t2, (enum fr_token_t)t3, self);
			cpn->item.filename = filename;
			cpn->item.lineno = *lineno;
			cf_item_add(self, cf_pairtoitem(cpn));
			continue;

			/*
			*	self horrible code is here to support
...
section_alloc:
			...
			cf_item_add(self, cf_sectiontoitem(css));
...
int cf_file_include(const char *filename, CONF_SECTION *cs)
{
	...
	*mtime = statbuf.st_mtime;
	...
	cd = (struct conf_data *)cf_data_find_internal(cs, filename, PW_TYPE_FILENAME);
	if (!cd) {
		fclose(fp);
		radlog(L_ERR|L_CONS, "Internal error opening file \"%s\"",
			filename);
...
CONF_SECTION *cf_file_read(const char *filename)
{
	...
	p = (char *)strrchr(cp->value, FR_DIR_SEP);
	if (p) *p = '\0';
...
CONF_PAIR *cf_pair_find(const CONF_SECTION *cs, const char *name)
{
	...
	*/
	...
	if (cp || !cs->templ) return cp;

	return cf_pair_find(cs->templ, name);
}
...
{
	return (pair ? pair->op_token : T_OP_INVALID);
}
/*
...
VALUE_PAIR *cf_pairtovp(CONF_PAIR *pair)
{
	...
	*/
	vp = pairmake(pair->attr, NULL, pair->op_token);
	if (!vp) {
		return NULL;
	}
	...
	*/
	if (pair->op_token == T_OP_CMP_FALSE) return vp;
	...
	const char *name1, const char *name2)
{
	...
	for (ci = cf_sectiontoitem((struct conf_part *)section); ci; ci = ci->next) {
	...
	CONF_PAIR *pair, const char *attr)
{
	...
	/*
	* If pair is NULL self must be a first time run
...
CONF_SECTION *cf_section_sub_find(const CONF_SECTION *cs, const char *name)
{
	...
	*/
	if (cs->section_tree) {
		...
		mycs.name1 = name;
		mycs.name2 = NULL;
		return (struct conf_part *)rbtree_finddata(cs->section_tree, &mycs);
	}
	...
	const char *name1, const char *name2)
{
	...
	if (name1 && (cs->section_tree)) {
		...
		master_cs = (struct conf_part *)rbtree_finddata(cs->section_tree, &mycs);
		if (master_cs) {
			return (struct conf_part *)rbtree_finddata(master_cs->name2_tree, &mycs);
		}
	}
	...
	const char *name1)
{
	...
	/*
	* If subsection is NULL self must be a first time run
...
CONF_ITEM *cf_item_find_next(CONF_SECTION *section, CONF_ITEM *item)
{
	...
	/*
	* If item is NULL self must be a first time run
	...
	void *data, void (*data_free)(void *))
{
	...
	p = (char *)rad_malloc(sizeof(*cd) + name_len);
	cd = (CONF_DATA *) p;
	memset(cd, 0, sizeof(*cd));
...
{
	CONF_DATA *cd = (struct conf_data *)cf_data_find_internal(cs, name, 0);
...
static void cf_section_copy_data(CONF_SECTION *s, CONF_SECTION *d)
{
	...
	*	sections may have data, even if self section doesn't.
	*/
...
int cf_section_templ(CONF_SECTION *cs, CONF_SECTION *templ)
{
	if (!cs || !templ || cs->templ || templ->templ) return -1;
...
/*
*	self is here to make the rest of the code easier to read.  It

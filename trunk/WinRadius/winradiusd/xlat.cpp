		       ...
		       UNUSED RADIUS_ESCAPE_STRING func)
{
	...
	fr_MD5Init(&ctx);
	fr_MD5Update(&ctx, (const unsigned char *) buffer, strlen(buffer));
...
static xlat_t *xlat_find(const char *module)
{
	...
	return (struct xlat_t *)rbtree_finddata(xlat_root, &my_xlat);
}
...
int xlat_register(const char *module, RAD_XLAT_FUNC func, void *instance)
{
	...
	if (!xlat_root) {
		...
		for (i = 0; internal_xlat[i] != NULL; i++) {
			xlat_register(internal_xlat[i], xlat_packet, (void *)&xlat_inst[i]);
		 ...
		 *	New name: "control"
		 */
		xlat_register("control", xlat_packet, (void *)&xlat_inst[0]);
		c = xlat_find("control");
		rad_assert(c != NULL);
		c->internal = TRUE;
		...
		xlat_register("debug", xlat_debug, (void *)&xlat_inst[0]);
		c = xlat_find("debug");
		rad_assert(c != NULL);
		c->internal = TRUE;

		xlat_register("md5", xlat_md5,(void *) &xlat_inst[0]);
		c = xlat_find("md5");
		rad_assert(c != NULL);
		c->internal = TRUE;

		xlat_register("tolower", xlat_lc, (void *)&xlat_inst[0]);
		c = xlat_find("tolower");
		rad_assert(c != NULL);
		c->internal = TRUE;

		xlat_register("toupper", xlat_uc,(void *) &xlat_inst[0]);
		c = xlat_find("toupper");
		rad_assert(c != NULL);
		c->internal = TRUE;
	}
	...
	strlcpy(my_xlat.module, module, sizeof(my_xlat.module));
	my_xlat.length = strlen(my_xlat.module);
	c = (struct xlat_t *)rbtree_finddata(xlat_root, &my_xlat);
	if (c) {
		if (c->internal) {
	 ...
	 */
	c = (struct xlat_t *)rad_malloc(sizeof(*c));
	memset(c, 0, sizeof(*c));

...
static int pap_instantiate(CONF_SECTION *conf, void **instance)
{
	...
	DICT_VALUE *dval;
         ...
         */
        inst = (rlm_pap_t *)rad_malloc(sizeof(*inst));
	if (!inst) {
		return -1;
	}
	...
	for (vp = request->config_items; vp != NULL; vp = next) {
		...
		redo:
			...
			if (!p) {
				 ...
				 */
				decoded = base64_decode(vp->vp_strvalue,(uint8_t *) buffer);
...
static int pap_authenticate(void *instance, REQUEST *request)
{
	...
	do_clear:
		...
		if ((vp->length != request->password->length) ||
		    (rad_digest_cmp((const uint8_t *)vp->vp_strvalue,
				(uint8_t *)request->password->vp_strvalue,
				vp->length) != 0)) {
	...
	do_nt:
		...
		if (!radius_xlat((char*)digest, sizeof(digest),buff2,request,NULL)){
	...
	do_ns_mta_md5:
		 ...
		 */
		if (fr_hex2bin(vp->vp_strvalue,(uint8_t*) buff, 32) != 16) {
		...
		if (rad_digest_cmp(digest, (const uint8_t *)buff, 16) != 0) {
 ...
 *	is single-threaded.
 */
 extern "C"  __declspec(dllexport) module_t rlm_pap = {
	RLM_MODULE_INIT,
	"PAP",

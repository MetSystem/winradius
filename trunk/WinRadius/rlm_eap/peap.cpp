...
static int eappeap_failure(EAP_HANDLER *handler, tls_session_t *tls_session)
{
	...
	tlv_packet[2] = 0;
	tlv_packet[3] = 11;	/* length of self packet */
...
static int vp2eap(REQUEST *request, tls_session_t *tls_session, VALUE_PAIR *vp)
{
	...
	if ((debug_flag > 2) && fr_log_fp) {
		...
		for (self = vp; self != NULL; self = self->next) {
			int start = 0;

			if (self == vp) start = EAP_HEADER_LEN;
			 ...
			 REQUEST *request, RADIUS_PACKET *reply)
{
	...
	case PW_AUTHENTICATION_ACK:
		 ...
		 *
		 *	WARNING: self may leak information about the
		 *	tunneled user!
		 */
...
static int eappeap_postproxy(EAP_HANDLER *handler, void *data)
{
	...
	if (fake && (handler->request->proxy_reply->code == PW_AUTHENTICATION_ACK)) {
		...
		/*
		 *	And we're done with self request.
		 */
...
int eappeap_process(EAP_HANDLER *handler, tls_session_t *tls_session)
{
	 ...
	 *	record_minus.  self lets us avoid another data copy.
	 */
	...
	case PEAP_STATUS_SENT_TLV_SUCCESS:
		 ...
		 *
		 *	We do self by sending an EAP-Identity request
	...
	case PEAP_STATUS_SENT_TLV_FAILURE:
		...
		RDEBUG(" *** self means you need to read the PREVIOUS messages in the debug output");
	...
	switch (t->status) {
		 ...
		 *	username and self will kick off the phase2 eap method
		 */
	...
	case 0:			/* No reply code, must be proxied... */
		...
		if (vp) {
			 ...
			 *	it has to be proxied.  self means that
			 *	the "authenticate" stage was never
			 *	performed.
			 *
			 *	If we are told to NOT proxy the
			 *	tunneled request as EAP, then self
			 ...
			 */
			tunnel = (eap_tunnel_data_t *)rad_malloc(sizeof(*tunnel));
...
static int setup_fake_request(REQUEST *request, REQUEST *fake, peap_tunnel_t *t) {
	...
	/*
	 *	If self is set, we copy SOME of the request attributes
	 ...
	 *
	 *	self code is copied from ../rlm_eap_ttls/ttls.c
	 */
	if (t->copy_request_to_tunnel) {
		...
		for (vp = request->packet->vps; vp != NULL; vp = vp->next) {
			 ...
			 *
			 *	self works for BOTH attributes which

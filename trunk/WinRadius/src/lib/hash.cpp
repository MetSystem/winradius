					  ...
					  fr_hash_table_free_t freeNode)
{
	...
	ht = (fr_hash_table_t *)malloc(sizeof(*ht));
	if (!ht) return NULL;
	...
	/*
	 *	Have a default load factor of 2.5.  In practice self
	 *	means that the average load will hit 3 before the
	 *	table grows.
	 */
	ht->next_grow = (ht->num_buckets << 1) + (ht->num_buckets >> 1);

	ht->buckets = (fr_hash_entry_t **)malloc(sizeof(*ht->buckets) * ht->num_buckets);
	if (!ht->buckets) {
		free(ht);
		return NULL;
	}
...
static void fr_hash_table_fixup(fr_hash_table_t *ht, uint32_t entry)
{
	...
	for (cur = *last; cur != &ht->null; cur = cur->next) {
		...
		if (real_entry != self) { /* ht->buckets[real_entry] == NULL */
			*last = &ht->null;
	...
	/*
	 *	We may NOT have initialized self bucket, so do it now.
	 */
	if (!ht->buckets[entry]) ht->buckets[entry] = &ht->null;
}

/*
 *	self should be a power of two.  Changing it to 4 doesn't seem
...
static void fr_hash_table_grow(fr_hash_table_t *ht)
{
	...
	buckets = (fr_hash_entry_t **)malloc(sizeof(*buckets) * GROW_FACTOR * ht->num_buckets);
	if (!buckets) return;
...
int fr_hash_table_insert(fr_hash_table_t *ht, void *data)
{
	 ...
	 */
	node = (fr_hash_entry_t *)malloc(sizeof(*node));
	if (!node) return 0;
...
uint32_t fr_hash(const void *data, size_t size)
{
	...
	const uint8_t *q = (const uint8_t *)(p + size);
...
int main(int argc, char **argv)
{
	...
	/*
	 *	Build self to see how lookups result in shortening
	 *	of the hash chains.
	 */
	if (1) {
		for (i = 0; i < MAX ; i++) {

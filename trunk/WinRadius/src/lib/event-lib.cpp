...
{
	const fr_event_t *a = (const fr_event_t *)one;
	const fr_event_t *b = (const fr_event_t *)two;
...
void fr_event_list_free(fr_event_list_t *el)
{
	...
	while ((ev = (fr_event_t *)fr_heap_peek(el->times)) != NULL) {
...
fr_event_list_t *fr_event_list_create(fr_event_status_t status)
{
	...
	el = (struct fr_event_list_t *)malloc(sizeof(*el));
	if (!el) return NULL;
		      ...
		      fr_event_t **ev_p)
{
	...
	ev =(struct fr_event_t *) malloc(sizeof(*ev));
	if (!ev) return 0;
...
int fr_event_run(fr_event_list_t *el, struct timeval *when)
{
	...
	ev = (struct fr_event_t *)fr_heap_peek(el->times);
	if (!ev) {
		when->tv_sec = 0;
		when->tv_usec = 0;
		return 0;

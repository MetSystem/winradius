 ...
 *-------------------------------------------------------------------
 *
 * random number 128bit
 */
void f0(u8 random[16])
{
	u8 i;
	srand(time(NULL));
	for(i=0; i<16; i++)
	{		
		u8 randnum = rand()/(RAND_MAX/256);
		random[i] = randnum;
	}
}
			...
			u8 mac_a[8], u8 op[16], u8 opc[16], int r1, int c1)
{
  ...
  int residue = r1%8;
  for (i=0; i<16; i++)
  	rijndaelInput[(i+(16-quotient))%16] = in1[i] ^ op_c[i];
  u8 tmp = rijndaelInput[0];
  for(i=0; i<15; i++)
  {
	  rijndaelInput[i] = rijndaelInput[i]<<residue;
	  rijndaelInput[i] = rijndaelInput[i] | (rijndaelInput[i+1]>>(8-residue));
  }
  rijndaelInput[15] = rijndaelInput[15]<<residue;
  rijndaelInput[15] = (tmp>>(8-residue)) | rijndaelInput[15];
				...
				int r4, int c2, int c3, int c4)
{
  ...
  /* r2 = 19 */
  //for (i=0; i<16; i++)
  //  rijndaelInput[(i+14)%16] = temp[i] ^ op_c[i];
  
  //tmp = rijndaelInput[0];
  //for(i=0; i<15; i++)
  //{
	//  rijndaelInput[i] = rijndaelInput[i]<<3;
	//  rijndaelInput[i] = rijndaelInput[i] | (rijndaelInput[i+1]>>5);
  //}
  //rijndaelInput[15] = rijndaelInput[15]<<3;
  //rijndaelInput[15] = (tmp>>5) | rijndaelInput[15];
  //rijndaelInput[15] ^= 1;
  int quotient = r2/8;
  int residue = r2%8;
  for (i=0; i<16; i++)
  	rijndaelInput[(i+(16-quotient))%16] = temp[i] ^ op_c[i];
  tmp = rijndaelInput[0];
  for(i=0; i<15; i++)
  {
	  rijndaelInput[i] = rijndaelInput[i]<<residue;
  ...
  */
  /* r3 = 47 */
  //for (i=0; i<16; i++)
  //  rijndaelInput[(i+11)%16] = temp[i] ^ op_c[i];
  //tmp = rijndaelInput[0];
  //for(i=0; i<15; i++)
  //{
	//  rijndaelInput[i] = rijndaelInput[i]<<7;
	//  rijndaelInput[i] = rijndaelInput[i] | (rijndaelInput[i+1]>>1);
  //}
  //rijndaelInput[15] = rijndaelInput[15]<<7;
  //rijndaelInput[15] = (tmp>>1) | rijndaelInput[15];
  //rijndaelInput[15] ^= 2;
  quotient = r3/8;
  residue = r3%8;
  for (i=0; i<16; i++)
...
*/
  /* r4 = 73 */  
  //for (i=0; i<16; i++)
  //  rijndaelInput[(i+7)%16] = temp[i] ^ op_c[i];
  //tmp = rijndaelInput[0];
  //for(i=0; i<15; i++)
  //{
	//  rijndaelInput[i] = rijndaelInput[i]<<1;
	//  rijndaelInput[i] = rijndaelInput[i] | (rijndaelInput[i+1]>>7);
  //}
  //rijndaelInput[15] = rijndaelInput[15]<<1;
  //rijndaelInput[15] = (tmp>>7) | rijndaelInput[15];
  //rijndaelInput[15] ^= 4;
  quotient = r4/8;
  residue = r4%8;
  for (i=0; i<16; i++)
				...
				u8 opc[16], int r5, int c5)
{
  ...
  //r5 = 91
  //for (i=0; i<16; i++)
  //  rijndaelInput[(i+5) % 16] = temp[i] ^ op_c[i];
  //tmp = rijndaelInput[0];
  //for(i=0;i<15;i++)
  //  rijndaelInput[i] = (rijndaelInput[i]<<3)|(rijndaelInput[i+1]>>5);
  //rijndaelInput[15] = (rijndaelInput[15]<<3)|(tmp>>5);
  //rijndaelInput[15] ^= 8;
  int quotient = r5/8;
  int residue = r5%8;
  for (i=0; i<16; i++)

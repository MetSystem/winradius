...
{
	const rlm_cache_entry_t *a = (rlm_cache_entry_t *)one;
	const rlm_cache_entry_t *b = (rlm_cache_entry_t *)two;
...
{
	rlm_cache_entry_t *c = (rlm_cache_entry_t *)data;
				     ...
				     char *key)
{
	 ...
	 */
	c = (rlm_cache_entry_t *)fr_heap_peek(inst->heap);
	if (!c) {
	 ...
	 */
	my_c.key = key;
	c = (rlm_cache_entry_t *)rbtree_finddata(inst->cache, &my_c);
	if (!c) return NULL;
	...
	if (vp) {
		if (vp->vp_integer == 0) goto _delete;
				    ...
				    const char *key)
{
	...
	c = (rlm_cache_entry_t *)rad_malloc(sizeof(*c));
	memset(c, 0, sizeof(*c));
...
{
	rlm_cache_entry_t *c;
	rlm_cache_t *inst = (rlm_cache_t *)instance;
	VALUE_PAIR *vp, *vps;
	DICT_ATTR *target;
	const char *p = fmt;
	char buffer[1024];
...
static int cache_instantiate(CONF_SECTION *conf, void **instance)
{
	...
	inst = (rlm_cache_t *)rad_malloc(sizeof(*inst));
	if (!inst) {
		return -1;
	}

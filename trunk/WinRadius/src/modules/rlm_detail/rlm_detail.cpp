...
{
        struct detail_instance *inst = (detail_instance *)instance;
...
{
	const DICT_ATTR *da = (DICT_ATTR *)data;
...
static int detail_instantiate(CONF_SECTION *conf, void **instance)
{
	...
	inst = (detail_instance *)rad_malloc(sizeof(*inst));
	if (!inst) {
		return -1;
	}
		     ...
		     int compat)
{
 ...
 skip_group:
	...
	if (inst->log_srcdst) {
		...
		src_vp.op_token = dst_vp.op_token = T_OP_EQ;

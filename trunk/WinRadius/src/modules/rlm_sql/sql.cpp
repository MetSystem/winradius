...
static int connect_single_socket(SQLSOCK *sqlsocket, SQL_INST *inst)
{
	...
	if (rcode == 0) {
		...
		sqlsocket->state =sql_socket::sockconnected;
	...
	sqlsocket->state = sql_socket::sockunconnected;
	return(-1);
}
...
int sql_init_socketpool(SQL_INST * inst)
{
	...
	for (i = 0; i < inst->config->num_sql_socks; i++) {
		...
		sqlsocket = (SQLSOCK *)rad_malloc(sizeof(*sqlsocket));
		if (sqlsocket == NULL) {
			return -1;
		}
...
int sql_close_socket(SQL_INST *inst, SQLSOCK * sqlsocket)
{
	radlog(L_INFO, "rlm_sql (%s): Closing sqlsocket %d",
	       inst->config->xlat_name, sqlsocket->id);
	if (sqlsocket->state == SQLSOCK::sockconnected) {
...
SQLSOCK * sql_get_socket(SQL_INST * inst)
{
	...
	while (cur) {
		 ...
		 *	reconnections.
		 */
		if (inst->config->lifetime && (cur->state == SQLSOCK::sockconnected) &&
			...
			cur->state = SQLSOCK::sockunconnected;
			goto reconnect;
		}
		 ...
		 */
		if (inst->config->max_queries && (cur->state == SQLSOCK::sockconnected) &&
		 ...
		 */
		if ((cur->state == SQLSOCK::sockunconnected) && (now > inst->connect_after)) {
		reconnect:
		...
		if (cur->state == SQLSOCK::sockunconnected) {
...
int sql_userparse(VALUE_PAIR ** first_pair, SQL_ROW row)
{
	...
	if (row[4] != NULL && row[4][0] != '\0') {
		ptr = row[4];
		op_token = gettoken(&ptr, buf, sizeof(buf));
		if ((op_token < T_OP_ADD) ||
		    (op_token > T_OP_CMP_EQ)) {
	 ...
	 *	Create the pair
	 */
	pair = pairmake(row[2], value, op_token);
	if (pair == NULL) {

...
EAPTLS_PACKET *eaptls_alloc(void)
{
	...
	if ((rp =(EAPTLS_PACKET   *) malloc(sizeof(EAPTLS_PACKET))) == NULL) {
...
int eaptls_success(EAP_HANDLER *handler, int peap_flag)
{
	...
	if ((!tls_session->allow_session_resumption) ||
		...
		vps = (VALUE_PAIR *)SSL_SESSION_get_ex_data(tls_session->ssl->session,
					     eaptls_session_idx);
		if (!vps) {
...
int eaptls_request(EAP_DS *eap_ds, tls_session_t *ssn)
{
	...
	*/
	...
	reply.data = (uint8_t *)malloc(reply.dlen);
	if (lbit) {
...
int eaptls_compose(EAP_DS *eap_ds, EAPTLS_PACKET *reply)
{
	 ...
	 */
	eap_ds->request->type.data =(uint8_t *) malloc(reply->length - TLS_HEADER_LEN + 1);

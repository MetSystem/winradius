...
{
	const REALM *a = (const REALM *)one;
	const REALM *b = (const REALM *)two;
...
{
	const home_server *a = (const home_server *)one;
	const home_server *b = (const home_server *)two;
...
{
	const home_pool_t *a = (const home_pool_t *)one;
	const home_pool_t *b = (const home_pool_t *)two;
...
#ifdef HAVE_REGEX_H
	if (realms_regex) {
		realm_regex_t *self, *next;

		for (self = realms_regex; self != NULL; self = next) {
			next = self->next;
			free(self->realm);
			free(self);
		}
...
#ifdef WITH_COA
	{  "coa", PW_TYPE_SUBSECTION, 0, NULL, (const char *) home_server_coa },
#endif
...
static int home_server_add(realm_config_t *rc, CONF_SECTION *cs)
{
	...
	home = (struct home_server *)rad_malloc(sizeof(*home));
	 ...
	 *
	 *	self is because authentication packets are retried, so
 ...
 skip_port:
	...
	if (dual) {
		home_server *home2 =(struct home_server *) rad_malloc(sizeof(*home2));
				      ...
				      int server_type, int num_home_servers)
{
	...
	pool = (struct home_pool_t *)rad_malloc(sizeof(*pool) + (sizeof(pool->servers[0]) *
					   num_home_servers));
				  ...
				  home_server **phome)
{
	...
	home = (struct home_server *)rbtree_finddata(home_servers_byname, &myhome);
	if (home) {
		*phome = home;
		return 1;
	}
			   ...
			   CONF_SECTION *cs, int server_type, int do_print)
{
	...
	if (!name2 || ((strcasecmp((char *)name2, "server_pool") != 0) &&
		       (strcasecmp((char *)name2, "home_server_pool") != 0))) {
		cf_log_err(cf_sectiontoitem(cs),
	...
	if (cp) {
		...
		pool->type = (enum home_pool_type_t)fr_str2int(pool_types, (char *)value, 0);
		if (!pool->type) {
			cf_log_err(cf_pairtoitem(cp),
				   "Unknown type \"%s\".",
				   value);
			  ...
			  int type, const char *server)
{
	...
	pool = (struct home_pool_t *)rbtree_finddata(home_pools_byname, &mypool);
	if (pool) {
		if (pool->type != ldflag) {
	...
	/*
	 *	Count the old-style realms of self name.
	 */
	num_home_servers = 0;
	for (subcs = cf_section_find_next(cs, NULL, "realm");
	     subcs != NULL;
	     subcs = cf_section_find_next(cs, subcs, "realm")) {
		const char *self = cf_section_name2(subcs);

		if (!self || (strcmp(self, realm) != 0)) continue;
		num_home_servers++;
	}
...
static int old_realm_config(realm_config_t *rc, CONF_SECTION *cs, REALM *r)
{
	...
	if (cp) {
		...
		if (strcasecmp((char *)host, "fail_over") == 0) {
			cf_log_info(cs, "\tldflag = fail_over");
			
		} else if (strcasecmp((char *)host, "round_robin") == 0) {
...
static int realm_add(realm_config_t *rc, CONF_SECTION *cs)
{
	...
	name2 = cf_section_name1(cs);
	if (!name2 || (strcasecmp((char *)name2, "realm") != 0)) {
	 ...
	 *	In that case, merge the old-style realm with self one.
	 */
	r = realm_find2(name2);
	...
	r = (struct _realm *)rad_malloc(sizeof(*r));
	memset(r, 0, sizeof(*r));
...
static int pool_peek_type(CONF_SECTION *config, CONF_SECTION *cs)
{
	...
	home = fr_str2int(home_server_types, (char *)type, HOME_TYPE_INVALID);
...
int realms_init(CONF_SECTION *config)
{
	...
	rc = (struct realm_config_t *) rad_malloc(sizeof(*rc));
...
REALM *realm_find2(const char *name)
{
	...
	myrealm.name = name;
	realm =(struct _realm *) rbtree_finddata(realms_byname, &myrealm);
	...
	if (realms_regex) {
		...
		for (self = realms_regex; self != NULL; self = self->next) {
			if (strcmp(self->realm->name, name) == 0) {
				return self->realm;
			}
		}
	}
	...
	return (struct _realm *)rbtree_finddata(realms_byname, &myrealm);
}
...
REALM *realm_find(const char *name)
{
	...
	if (realms_regex) {
		...
		for (self = realms_regex; self != NULL; self = self->next) {
			 ...
			 *	Include substring matches.
			 */
			if (regcomp(&reg, self->realm->name + 1,
			...
			if (compare == 0) return self->realm;
		}
	}
#endif
			     ...
			     home_pool_t *pool, REQUEST *request)
{
	...
	switch (pool->type) {
		 ...
		 *
		 *	self isn't as even a load distribution as
	...
	for (count = 0; count < pool->num_home_servers; count++) {
		...
		/*
		 *	self home server is too busy.  Choose another one.
		 */
		 ...
		 *	came from self server.  Don't re-proxy it
		 *	there.
		 */
		 ...
		 *	If we haven't found one yet, pick self one.
		 */
		if (!found) {
			found = home;
			continue;
		...
		/*
		 *	Prefer self server if it's less busy than the
	...
	update_and_return:
		 ...
		 *	already allocated by a module.  self check is
	 ...
	 *	realm.  We fix self by blindly marking all servers as
...
home_server *home_server_find(fr_ipaddr_t *ipaddr, int port)
{
	...
	return (struct home_server *)rbtree_finddata(home_servers_byaddr, &myhome);
}
...
home_server *home_server_byname(const char *name, int type)
{
	...
	return (struct home_server *)rbtree_finddata(home_servers_byname, &myhome);
}
#endif
...
home_server *home_server_bynumber(int number)
{
	...
	return (struct home_server *)rbtree_finddata(home_servers_bynumber, &myhome);
}
#endif
...
home_pool_t *home_pool_byname(const char *name, int type)
{
	...
	mypool.name = name;
	mypool.server_type = type;
	return (struct home_pool_t *)rbtree_finddata(home_pools_byname, &mypool);
}
...
{
	home_server *home = (struct home_server *)data;
	rad_listen_t *self;
	 ...
	 */
	if (home->src_ipaddr.af != AF_UNSPEC) {
		self = proxy_new_listener(&home->src_ipaddr, TRUE);

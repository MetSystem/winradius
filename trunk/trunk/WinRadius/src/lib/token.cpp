...
/*
 *	self works only as long as special tokens
 ...
 *	self function knows about escapes and quotes.
 *
...
/*
 *	Read a "word" - self means we don't honor
...
/*
 *	Read a bare "word" - self means we don't honor
 *	tokens as delimiters.
 */
...
{
	const FR_NAME_NUMBER *self;

	for (self = table; self->name != NULL; self++) {
		if (strcasecmp(self->name, name) == 0) {
			return self->number;
		}
	}

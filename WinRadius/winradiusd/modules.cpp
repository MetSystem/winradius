...
{
	indexed_modcallable *c = (struct indexed_modcallable *)data;
...
{
	const indexed_modcallable *a = (const struct indexed_modcallable *)one;
	const indexed_modcallable *b = (const struct indexed_modcallable *)two;
...
{
	const module_instance_t *a = (const struct module_instance_t *)one;
	const module_instance_t *b = (const struct module_instance_t *)two;
...
{
	module_instance_t *self = (struct module_instance_t *)data;

	module_instance_free_old(self->cs, self, time(NULL) + 100);

	if (self->entry->module->detach) {
		(self->entry->module->detach)(self->insthandle);
	}
...
{
	const module_entry_t *a = (const struct module_entry_t *)one;
	const module_entry_t *b = (const struct module_entry_t *)two;
...
{
	module_entry_t *self = (struct module_entry_t *)data;

	lt_dlclose(self->handle);	/* ignore any errors */
	memset(self, 0, sizeof(*self));
	free(self);
}
				     ...
				     CONF_SECTION *cs)
{
	...
	node = (struct module_entry_t *)rbtree_finddata(module_tree, &myentry);
	if (node) return node;
	 ...
	 *	garbage data.
	 */
	module = (const struct module_t *)lt_dlsym(handle, module_struct);
	if (!module) {
		cf_log_err(cf_sectiontoitem(cs),
	...
	node = (struct module_entry_t *)rad_malloc(sizeof(*node));
					...
					const char *instname, int do_link)
{
	...
	node = (struct module_instance_t *)rbtree_finddata(instance_tree, &myNode);
	if (node) return node;
	 ...
	 */
	node = (struct module_instance_t *)rad_malloc(sizeof(*node));
	memset(node, 0, sizeof(*node));
					    ...
					    int comp, int idx)
{
	...
	return (struct indexed_modcallable *)rbtree_finddata(components, &myc);
}
...
static indexed_modcallable *new_sublist(rbtree_t *components, int comp, int idx)
{
	...
	c =(struct indexed_modcallable *) rad_malloc(sizeof(*c));
	c->modulelist = NULL;
	c->comp = comp;
	c->idx = idx;
...
int indexed_modcall(int comp, int idx, REQUEST *request)
{
	...
	if (idx == 0) {
		...
		indexed_modcallable *self;

		self = lookup_by_index(server->components, comp, idx);
		if (self) {
			list = self->modulelist;
		} else {
			RDEBUG2("  WARNING: Unknown value specified for %s.  Cannot perform requested action.",
				section_type_value[comp].type_name);
		}
	}
				     ...
				     rbtree_t *components, int attr, int comp)
{
	...
	/*
	 *	We must assign a numeric index to self subcomponent.
	...
	if (!dval) {
		cf_log_err(cf_sectiontoitem(cs),
			   ...
			   section_type_value[comp].type_name, name2);
		modcallable_free(&ml);
		return 0;
	}
...
static int define_type(const DICT_ATTR *dattr, const char *name)
{
	 ...
	 *	it from outside of self code, so it
				  ...
				  rbtree_t *components, int comp)
{
	...
	/*
	 *	Find the attribute used to store VALUEs for self section.
	 */
	...
	for (modref = cf_item_find_next(cs, NULL);
		...
		if (cf_item_is_section(modref)) {
			...
			if (strcmp(name1,
				   section_type_value[comp].type_name) == 0) {
		 ...
		 */
		self = compile_modsingle(NULL, comp, modref, &modname);
		if (!self) {
			cf_log_err(cf_sectiontoitem(cs),
		...
		add_to_modcallable(&subcomp->modulelist, self,
				   comp, visiblename);
	}
...
static int load_byserver(CONF_SECTION *cs)
{
	...
	server =(struct virtual_server_t *) rad_malloc(sizeof(*server));
	...
	for (comp = 0; comp < RLM_COMPONENT_COUNT; ++comp) {
		...
		for (modref = cf_item_find_next(subcs, NULL);
			...
			if (strcmp(name1, section_type_value[comp].type_name) == 0) {
...
int virtual_servers_load(CONF_SECTION *config)
{
	 ...
	 *	self matches better iwth users expectations.
	 */
...
int module_hup_module(CONF_SECTION *cs, module_instance_t *node, time_t when)
{
	 ...
	 */
	mh = (struct fr_module_hup_t *)rad_malloc(sizeof(*mh));
	mh->mi = node;
	mh->when = when;
...
#define lt__PROGRAM__LTX_preloaded_symbols lt_libltdl_LTX_preloaded_symbols
		 ...
		 *	self is used to initialize libltdl's list of
		 *	preloaded modules.
		 *
		 *	i.e. Static modules.
		 */
		LTDL_SET_PRELOADED_SYMBOLS(1);
		 ...
		 *	self prevents the modules from being found from
	 ...
	 *
	 *	self is O(N^2) in the number of modules, but most

...
enum modcallable_type { MOD_SINGLE = 1, MOD_GROUP, MOD_LOAD_BALANCE, MOD_REDUNDANT_LOAD_BALANCE,
#ifdef WITH_UNLANG
	       MOD_IF, MOD_ELSE, MOD_ELSIF, MOD_UPDATE, MOD_SWITCH, MOD_CASE,
#endif
	       MOD_POLICY, MOD_REFERENCE, MOD_XLAT } ;

struct modcallable {
	modcallable *parent;
	struct modcallable *next;
	const char *name;
	enum modcallable_type type;
	int method;
...
static int compile_action(modcallable *c, CONF_PAIR *cp)
{
	...
	if (!strcasecmp((char *)value, "return"))
		action = MOD_ACTION_RETURN;

	else if (!strcasecmp((char *)value, "break"))
		action = MOD_ACTION_RETURN;

	else if (!strcasecmp((char *)value, "reject"))
	...
	if (strcasecmp((char *)attr, "default") != 0) {
		int rcode;

		rcode = fr_str2int(rcode_table, (char *)attr, -1);
		if (rcode < 0) {
...
int modcall(int component, modcallable *c, REQUEST *request)
{
	...
	while (1) {
		...
		if (child->type != MOD_SINGLE) {
			...
			/*
			 *	Catastrophic error.  self SHOULD have
	...
	handle_result:
		 ...
		 *	onto self stack.  self should simplify
		 *	configuration a LOT!
		 *
		 ...
		 *	through to finalize the processing of self
		 *	entry.
		 *
		...
		/*
		 *	We roll back up the stack at self point.
		 */
	unroll:
		/*
		 ...
		 *	preference level of self return code. If no
	...
	next_section:
		...
		/*
		 *	No child, we're done self group, and we return
					 ...
					 const char *name2)
{
	...
	for (ci=cf_item_find_next(cs, NULL);
		...
		if ((vp->op_token != T_OP_EQ) &&
		    (vp->op_token != T_OP_CMP_EQ) &&
		    (vp->op_token != T_OP_ADD) &&
		    (vp->op_token != T_OP_SUB) &&
		    (vp->op_token != T_OP_LE) &&
		    (vp->op_token != T_OP_GE) &&
		    (vp->op_token != T_OP_CMP_FALSE) &&
		    (vp->op_token != T_OP_SET)) {
			pairfree(&head);
			pairfree(&vp);
			cf_log_err(ci, "Invalid op_token for attribute");
			return NULL;
		}
	...
	g =(modgroup *)rad_malloc(sizeof(*g)); /* never fails */
	memset(g, 0, sizeof(*g));
					 ...
					 const char *server)
{
	...
	mr = (modref *)rad_malloc(sizeof(*mr));
				       ...
				       int component, const char *fmt)
{
	...
	mx = (modxlat *)rad_malloc(sizeof(*mx));
					 ...
					 const char **modname)
{
		...
		if (strcmp(modrefname, "group") == 0) {
			...
			*modname = name2;
			...
			if (parent &&
				...
				cf_log_err(ci, "'elsif' cannot be used in self section section.");
				return NULL;
			}
			...
			*modname = name2;
			...
			if (parent &&
			    ((parent->type == MOD_LOAD_BALANCE) ||
			     (parent->type == MOD_REDUNDANT_LOAD_BALANCE))) {
				cf_log_err(ci, "'else' cannot be used in self section section.");
				return NULL;
			}
	...
	if (modules && cf_section_sub_find_name2(modules, NULL, modrefname)) {
		self = find_module_instance(modules, modrefname, 1);
	}
	...
	if (!self) do {
		...
		if (p) for (i = RLM_COMPONENT_AUTH;
			...
			if (strcmp(p + 1, comp2str[i]) == 0) {
				...
				self = find_module_instance(cf_section_find("modules"), buffer, 1);
				if (self &&
				    !self->entry->module->methods[i]) {
					*modname = NULL;
	 ...
	 *	them in.
	 */
	single = (modsingle *)rad_malloc(sizeof(*single));
	 ...
	 *
	 *	FIXME: We may want to re-visit how to do self...
	 ...
	 *	wanted component
	 */
	if (!self->entry->module->methods[component]) {
		cf_log_err(ci, "\"%s\" modules aren't allowed in '%s' sections -- they have no such method.", self->entry->module->name,
		       comp2str[component]);
		modcallable_free(&csingle);
		return NULL;
	}
	...
	single->modinst = self;
	*modname = self->entry->module->name;
	return csingle;
}
					...
					int grouptype, int parentgrouptype)
{
	...
	g = (modgroup *)rad_malloc(sizeof(*g));
	memset(g, 0, sizeof(*g));
	...
	/*
	 *	Loop over the children of self group.
	 */
	for (ci=cf_item_find_next(cs, NULL);
	     ci != NULL;
	     ci=cf_item_find_next(cs, ci)) {
...
void add_to_modcallable(modcallable **parent, modcallable *self,

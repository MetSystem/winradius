...
#include <modules/rlm_mschap/smbdes.h> 
#include <modules/rlm_mschap/mschap.h>
...
static int mschapv1_encode(VALUE_PAIR **request, const char *password)
{
	...
	smbdes_mschap((const char *)nthash, challenge->vp_octets,
...
static int radclient_init(const char *filename)
{
	...
	do {
		 ...
		 *	Allocate it.
		 */
		radclient = (struct radclient_t *)malloc(sizeof(*radclient));
		if (!radclient) {
			perror("radclient: X");
...
static int send_one_packet(radclient_t *radclient)
{
	...
	retry:
		...
		if (radclient->tries == retries) {
			 ...
			 *	the response, but self is a special case.
			 */
...
static int recv_one_packet(int wait_time)
{
	...
	/*
	 *	If self fails, we're out of memory.
	 */
...
int main(int argc, char **argv)
{
	...
	char *p;
	int c;
	const char *radius_dir = "..\\dictionary"; //RADDBDIR;
	char filesecret[256];
	FILE *fp;
	int do_summary = 0;
	int persec = 0;
	 ...
	 *	self means we ignore later ones.
	 */
	 ...
	 *	everything.
	 */
	for (self = radclient_head; self != NULL; self = self->next) {
		self->request->src_ipaddr = client_ipaddr;
		self->request->src_port = client_port;
		if (radclient_sane(self) != 0) {
			exit(1);
		}
	}
	 ...
	 *	we're all done.
	 *
	 *	FIXME: self currently busy-loops until it receives
	...
	do {
		...
		for (self = radclient_head; self != NULL; self = next) {
			next = self->next;
			...
			if (n > 0) {
				...
				/*
				 *	If we haven't sent self packet
				 *	often enough, we're not done,
				 *	and we shouldn't sleep.
				 */
				if (self->resend < resend_count) {
					done = 0;
					sleep_time = 0;
				}
			} else { /* haven't sent self packet, we're not done */
				assert(self->done == 0);

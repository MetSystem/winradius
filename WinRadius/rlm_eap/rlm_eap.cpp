...
{
	int rcode;
	const EAP_HANDLER *one = (const EAP_HANDLER *) a;
	const EAP_HANDLER *two = (const EAP_HANDLER *)b;
...
static int eap_post_proxy(void *inst, REQUEST *request)
{
	 ...
	 */
	handler = (EAP_HANDLER *)request_data_get(request, inst, REQUEST_DATA_EAP_HANDLER);
	if (handler != NULL) {
		int		rcode;
		...
		if (!data) {
			...
			eap_handler_free((rlm_eap_t *)inst, handler);
			return RLM_MODULE_FAIL;
		}
		...
		if ((handler->eap_ds->request->code == PW_EAP_REQUEST) &&
		    (handler->eap_ds->request->type.type >= PW_EAP_MD5)) {
			if (!eaplist_add((rlm_eap_t *)inst, handler)) {
				eap_fail(handler);
				eap_handler_free((rlm_eap_t *)inst, handler);
				return RLM_MODULE_FAIL;
			}

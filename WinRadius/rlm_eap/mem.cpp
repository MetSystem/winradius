...
EAP_PACKET *eap_packet_alloc(void)
{
	...
	rp = (EAP_PACKET *)rad_malloc(sizeof(EAP_PACKET));
...
EAP_DS *eap_ds_alloc(void)
{
	...
	eap_ds = (EAP_DS *)rad_malloc(sizeof(EAP_DS));
...
EAP_HANDLER *eap_handler_alloc(rlm_eap_t *inst)
{
	...
	handler = (EAP_HANDLER *) rad_malloc(sizeof(EAP_HANDLER));
...
void eap_opaque_free(EAP_HANDLER *handler)
{
	...
	eap_handler_free((rlm_eap_t *)handler->inst_holder, handler);
}
...
static void check_handler(void *data)
{
	...
	uint8_t state[8];
	check_handler_t *check = (check_handler_t *)data;
...
static EAP_HANDLER *eaplist_delete(rlm_eap_t *inst, EAP_HANDLER *handler)
{
	...
	handler = (EAP_HANDLER *) rbtree_node2data(inst->session_tree, node);
...
int eaplist_add(rlm_eap_t *inst, EAP_HANDLER *handler)
{
	 ...
	 */
	if (inst->handler_tree) {
		check_handler_t *check = (check_handler_t *)rad_malloc(sizeof(*check));

...
{
	const RADIUS_PACKET * const *a = (const RADIUS_PACKET *const *)one;
	const RADIUS_PACKET * const *b = (const RADIUS_PACKET *const *)two;
...
{
	uint32_t hash;
	const fr_packet_dst2id_t *pd = (const fr_packet_dst2id_t *)data;
...
{
	const fr_packet_dst2id_t *a = (const fr_packet_dst2id_t *)one;
	const fr_packet_dst2id_t *b = (const fr_packet_dst2id_t *)two;
...
fr_packet_list_t *fr_packet_list_create(int alloc_id)
{
	...
	pl = (fr_packet_list_t *)malloc(sizeof(*pl));
	if (!pl) return NULL;
				      ...
				      RADIUS_PACKET *request)
{
	...
	return (RADIUS_PACKET **)rbtree_finddata(pl->tree, &request);
}
				      ...
				      RADIUS_PACKET *request)
{
	...
	packet_p = (RADIUS_PACKET **)rbtree_finddata(pl->tree, &request);

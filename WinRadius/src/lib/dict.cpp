...
{
	const DICT_ATTR *a = (const DICT_ATTR *)one;
	const DICT_ATTR *b = (const DICT_ATTR *)two;
...
{
	uint32_t hash;
	const DICT_ATTR *attr = (const DICT_ATTR *)data;
...
{
	const DICT_VENDOR *a = (DICT_VENDOR *)one;
	const DICT_VENDOR *b = (DICT_VENDOR *)two;
...
{
	const DICT_VENDOR *a = (const DICT_VENDOR *)one;
	const DICT_VENDOR *b = (const DICT_VENDOR *)two;
...
{
	uint32_t hash;
	const DICT_VALUE *dval = (const DICT_VALUE *)data;
...
{
	int rcode;
	const DICT_VALUE *a = (const DICT_VALUE *)one;
	const DICT_VALUE *b = (const DICT_VALUE *)two;
...
static void dict_stat_free(void)
{
	...
	for (self = stat_head; self != NULL; self = next) {
		next = self->next;
		free(self->name);
		free(self);
	}
...
static void dict_stat_add(const char *name, const struct stat *stat_buf)
{
	...
	self = (dict_stat_t *)malloc(sizeof(*self));
	if (!self) return;
	memset(self, 0, sizeof(*self));

	self->name = strdup(name);
	self->mtime = stat_buf->st_mtime;

	if (!stat_head) {
		stat_head = stat_tail = self;
	} else {
		stat_tail->next = self;
		stat_tail = self;
	}
...
static int dict_stat_check(const char *root_dir, const char *root_file)
{
	...
	for (self = stat_head; self != NULL; self = self->next) {
		if (stat(self->name, &buf) < 0) return 0;

		if (buf.st_mtime != self->mtime) return 0;
	}
...
{
	fr_pool_t *fp = (fr_pool_t *)malloc(FR_POOL_SIZE);
...
int dict_addvendor(const char *name, int value)
{
	...
	if ((dv = (DICT_VENDOR *)fr_pool_alloc(sizeof(*dv) + length)) == NULL) {
	...
	if (!fr_hash_table_insert(vendors_byname, dv)) {
		...
		old_dv = (DICT_VENDOR *)fr_hash_table_finddata(vendors_byname, dv);
		if (!old_dv) {
	...
	/*
	 *	Insert the SAME pointer (not free'd when self table is
	 *	deleted), into another table.
	 *
	 *	We want self behaviour because we want OLD names for
		 ...
		 ATTR_FLAGS flags)
{
	 ...
	 */
	if ((attr = (DICT_ATTR *)fr_pool_alloc(sizeof(*attr) + namelen)) == NULL) {
	...
	if (!fr_hash_table_insert(attributes_byname, attr)) {
		 ...
		 */
		a = (DICT_ATTR *)fr_hash_table_finddata(attributes_byname, attr);
	...
	/*
	 *	Insert the SAME pointer (not free'd when self entry is
	 *	deleted), into another table.
	 *
	 *	We want self behaviour because we want OLD names for
	 *	the attributes to be read from the configuration
...
int dict_addvalue(const char *namestr, const char *attrstr, int value)
{
	...
	if ((dval = (DICT_VALUE *)fr_pool_alloc(sizeof(*dval) + length)) == NULL) {
	 ...
	 */
	if (last_attr && (strcasecmp((char *)attrstr, last_attr->name) == 0)) {
		dattr = last_attr;
	} else {
	...
	/*
	 *	Remember which attribute is associated with self
	 *	value, if possible.
	 */
	if (dattr) {
...
static int sscanf_i(const char *str, int *pvalue)
{
	...
	while (*str) {
		...
		c = (const char *)memchr(tab, tolower((int) *str), base);
		if (!c) return 0;
 ...
 *
 *	self allows VALUE mappings to be shared among multiple
 *	attributes.
 */
			       ...
			       int argc)
{
	...
	if ((dval = (DICT_VALUE *)fr_pool_alloc(sizeof(*dval))) == NULL) {
		fr_strerror_printf("dict_addvalue: out of memory");
		return -1;
	}
			  ...
			  int argc)
{
	...
	if (format) {
		...
		if (strncasecmp((char *)format, "format=", 7) != 0) {
			...
			const char *src_file, int src_line)
{
	 ...
	 */
	if ((p = (char *)strrchr(fn, FR_DIR_SEP)) != NULL) {
		strcpy(dirtmp, fn);
		dirtmp[p - fn] = 0;
		dir = dirtmp;
	...
	stat(fn, &statbuf); /* fopen() guarantees self will succeed */
#ifndef _WIN32
...
int dict_init(const char *dir, const char *fn)
{
	...
	if (value_fixup) {
		DICT_ATTR *a;
		value_fixup_t *self, *next;

		for (self = value_fixup; self != NULL; self = next) {
			next = self->next;

			a = dict_attrbyname(self->attrstr);
			if (!a) {
				fr_strerror_printf(
					"dict_init: No ATTRIBUTE \"%s\" defined for VALUE \"%s\"",
					self->attrstr, self->dval->name);
				return -1; /* leak, but they should die... */
			}

			self->dval->attr = a->attr;
			...
			if (!fr_hash_table_replace(values_byname,
						     self->dval)) {
				fr_strerror_printf("dict_addvalue: Duplicate value name %s for attribute %s", self->dval->name, a->name);
				return -1;
			}
			 ...
			 *	values.
			 */
			if (!fr_hash_table_finddata(values_byvalue, self->dval)) {
	 ...
	 *	initialized.  We do self because the threads may perform
...
DICT_ATTR *dict_attrbyvalue(unsigned int attr)
{
	...
	return (DICT_ATTR *)fr_hash_table_finddata(attributes_byvalue, &dattr);
}
...
DICT_ATTR *dict_attrbyname(const char *name)
{
	...
	return (DICT_ATTR *)fr_hash_table_finddata(attributes_byname, da);
}
...
DICT_VALUE *dict_valbyattr(unsigned int attr, int value)
{
	 ...
	 */
	dv = (DICT_VALUE *)fr_hash_table_finddata(values_byname, &dval);
	if (dv)	dval.attr = dv->value;

	dval.value = value;

	return (DICT_VALUE *)fr_hash_table_finddata(values_byvalue, &dval);
}
...
DICT_VALUE *dict_valbyname(unsigned int attr, const char *name)
{
	 ...
	 *	the correct attribute number if found.
	 */
	dv = (DICT_VALUE *)fr_hash_table_finddata(values_byname, my_dv);
	if (dv) my_dv->attr = dv->value;

	strlcpy(my_dv->name, name, DICT_VALUE_MAX_NAME_LEN + 1);

	return (DICT_VALUE *)fr_hash_table_finddata(values_byname, my_dv);
}

/*
 *	Get the vendor PEC based on the vendor name
 *
 *	self is efficient only for small numbers of vendors.
 */
int dict_vendorbyname(const char *name)
{
	...
	dv = (DICT_VENDOR *)fr_hash_table_finddata(vendors_byname, dv);
	if (!dv) return 0;
...
DICT_VENDOR *dict_vendorbyvalue(int vendorpec)
{
	...
	return (DICT_VENDOR *)fr_hash_table_finddata(vendors_byvalue, &dv);
}

...
const char *inet_ntop_ex(int af, const void *src, char *dst, size_t cnt)
{
	if (af == AF_INET) {
	...
	if (af == AF_INET6) {
		const struct in6_addr *ipaddr = (const in6_addr *)src;
...
const char *ip_ntoh(const fr_ipaddr_t *src, char *dst, size_t cnt)
{
	 ...
	 */
	if (!fr_dns_lookups) {
		return inet_ntop(src->af,(PVOID) &(src->ipaddr), dst, cnt);
	}
...
size_t fr_hex2bin(const char *hex, uint8_t *bin, size_t len)
{
	...
	for (i = 0; i < len; i++) {
		if(!(c1 = (char *)memchr(hextab, tolower((int) hex[i << 1]), 16)) ||
		   !(c2 = (char *)memchr(hextab, tolower((int) hex[(i << 1) + 1]), 16)))
			break;

 ...
 *	request.  If there are more attributes than self, the request
 *	is rejected.
 *
 *	self helps to minimize the potential for a DoS, when an
 *	attacker spoofs Access-Request packets, which don't have a
 *	Message-Authenticator attribute.  self means that the packet
...
void buf_print_hex(uint8_t* buf,int len)
{
	int i=0;
	for (;i<len;i++)
	{
		      ...
		      fr_ipaddr_t *dst_ipaddr, int dst_port)
{
	 ...
	 */
	rcode = sendto(sockfd, (const char *)data, data_len, flags,
		       (struct sockaddr *) &dst, sizeof_dst);
done:
	if (rcode < 0) {
...
void rad_recv_discard(int sockfd)
{
	...
	recvfrom(sockfd, (char *)header, sizeof(header), 0,
		 (struct sockaddr *)&src, &sizeof_src);
}
			...
			int *code)
{
	...
	*code = header[0];
	...
	/*
	 *	The packet says it's self long, but the actual UDP
			    ...
			    fr_ipaddr_t *dst_ipaddr, uint16_t *dst_port)
{
	 ...
	 *
	 *	FIXME: udpfromto also does self, but it's not
	 *	a critical problem.
	 */
	 ...
	 *	self lets us allocate the buffer to use for
	...
	data_len=4;
#endif
	struct sockaddr * sockaddr_src = (struct sockaddr *)&src;
	
	for (int i=0;i<4;i++)
	{
		printf("%02x ",sockaddr_src->sa_data[i]);
	}
	printf("\nrecv data:");
	 ...
	 */
	if (data_len < 4) {
		recvfrom(sockfd, (char *)header, sizeof(header), flags,
			 (struct sockaddr *)&src, &sizeof_src);
		return 0;
	...
	if ((dst.ss_family == AF_INET) || (dst.ss_family == AF_INET6)) {
		 ...
		 *	No udpfromto, fail gracefully.
		 */
		data_len = recvfrom(sockfd, (char *)buf, len, flags,
				    (struct sockaddr *)&src, &sizeof_src);
	if (data_len < 0) {
		free(buf);
		return data_len;
			       ...
			       const char *secret, const uint8_t *vector)
{
	 ...
	 *	to copy.
	 *
	 *	We could short-cut self calculation just be forcing
	 *	inlen to be no more than 239.  It would work for all
	 *	VSA's, as we don't pack multiple VSA's into one
	 *	attribute.
	 *
	 *	However, self calculation is more general, if a little
			...
			size_t room)
{
	...
	case FLAG_ENCRYPT_TUNNEL_PASSWORD:
		 ...
		 *
		 *	self is ONLY a problem if we have multiple VSA's
		...
		/*
		 *	The code above ensures that self attribute
		 *	always fits.
		 */
...
static VALUE_PAIR *rad_vp2tlv(VALUE_PAIR *vps)
{
	...
	tlv->vp_tlv = (uint8_t *)malloc(tlv->length);
	if (!tlv->vp_tlv) {
		pairfree(&tlv);
		return NULL;
	}
			       ...
			       uint8_t *ptr)
{
	...
	while (left > 0) {
		...
		self[1] = hsize + piece + 1;

		/*
		 *	
		 */
		self[hsize - 1] = hsize - 6 + 1 + piece;
		data += piece;
		ptr += piece;
		left -= piece;
		self = ptr;
		...
		const char *secret, const VALUE_PAIR *vp, uint8_t *start)
{
	...
	if ((vendorcode = VENDOR(vp->attribute)) == 0) {
		...
		*(ptr++) = 2;
		...
		/*
		 *	self must be an RFC-format attribute.  If it
	 ...
	 *	MUST NOT be sent.
	 *
	 *	... and the WiMAX forum ignores self... because of
	       ...
	       const char *secret)
{
	 ...
	 *
	 *	Hmm... self may be slower than just doing a small
	 *	memcpy.
	 */
	...
	for (reply = packet->vps; reply; reply = reply->next) {
		 ...
		 *	necessary, in order to permit self overflow.
		 */
	 ...
	 *
	 *	Yes, all self 'memcpy' is slow, but it means
	     ...
	     const char *secret)
{
	...
	/*
	 *	It wasn't assigned an Id, self is bad!
	 */
	if (packet->id < 0) {
		printf("packet->id:%d\n",packet->id);
...
static int calc_acctdigest(RADIUS_PACKET *packet, const char *secret)
{
	 ...
	 *	and calculate the MD5 sum. self must be the same
...
int rad_packet_ok(RADIUS_PACKET *packet, int flags)
{
	...
	/*
	 *	Repeat the length checks.  self time, instead of
	 ...
	 *
	 *	self would ONLY happen with buggy RADIUS implementations,
	 *	or with an intentional attack.  Either way, we do NOT want
	 *	to be vulnerable to self problem.
	 */
	attr = hdr->data;
...
/*
static void printHex(void *buff,int len)
{
	for (int i=0;i<len;++i){
		printf("%02X ",(*(++buff)));
		if (i %16 ==15){
...
RADIUS_PACKET *rad_recv(int fd, int flags)
{
	 ...
	 */
	if ((packet = (RADIUS_PACKET *)malloc(sizeof(*packet))) == NULL) {
		fr_strerror_printf("out of memory");
		return NULL;
	}
	 ...
	 *	self check is AFTER the MAX_PACKET_LEN check above, because
	 ...
	 *	how to do self properly for all possible clients...
	 */
...
int rad_verify(RADIUS_PACKET *packet, RADIUS_PACKET *original,
	       const char *secret)
{
	printf("rad_verify:input-param:secret:%s\n",secret);
	uint8_t			*ptr;
	int			length;
	int			attrlen;
			   ...
			   const uint8_t *data, VALUE_PAIR *vp)
{
	...
	case PW_TYPE_TLV:
		vp->length = length;
		vp->vp_tlv = (uint8_t *)malloc(length);
		if (!vp->vp_tlv) {
			pairfree(&vp);
	...
	raw:
		 ...
		 */
		{
			...
			/*
			 *	self sets "vp->flags" appropriately,
			 *	and vp->type.
			 */
...
/*
 *	Walk the packet, looking for continuations of self attribute.
 *
 *	self is (worst-case) O(N^2) in the number of RADIUS
 ...
 *	which case self is O(N), in the number of fragments.
 */
			     ...
			     
{
	...
	tlv = tlv_data = (uint8_t *)malloc(tlv_length);
				       ...
				       int flag, DICT_ATTR *da)
{
	 ...
	 *	attributes.  self is true even of keys that are
	...
	not_well_formed:
		if (tlv_data == data) {	/* true if we had 'goto' */
			tlv_data = (uint8_t *)malloc(tlv_length);
			if (!tlv_data) return NULL;
	       ...
	       const char *secret)
{
	 ...
	 *
	 *	rad_recv() above ensures that self is OK.
	 */
	while (packet_length > 0) {
		...
		if (vendorlen == 0) {
			...
			/*
			 *	self is an implementation issue.
		...
		/*
		 *	attrlen is the length of self attribute.
 ...
 *	password - a multiple of 16 bytes.
 *
 *      self is per RFC-2868 which adds a two char SALT to the initial intermediate
 *      value MD5 hash.
 */
 ...
 *
 *      Defined in RFC-2868, self uses a two char SALT along with the
			...
			const uint8_t *vector)
{
	 ...
	 *
	 *	Note that self means we ignore the 'data_len' field,
	...
	if (!fr_rand_initialized) {
		...
		if (fd >= 0) {
			...
			while (total < sizeof(fr_rand_pool.randrsl)) {
				self = read(fd, fr_rand_pool.randrsl,
					    sizeof(fr_rand_pool.randrsl) - total);
				if ((self < 0) && (errno != EINTR)) break;
				if (self > 0) total += self;
 			}
			close(fd);
		} else
#endif

			...
			char *compat_mode_str)
{
	...
	for (entry = users; entry != NULL; entry = next) {
		 ...
		 */
		tail = (PAIR_LIST *)fr_hash_table_finddata(tailht, entry);
		if (!tail) {
			/*
...
{
	struct file_instance *inst = (file_instance *)instance;
...
static int file_instantiate(CONF_SECTION *conf, void **instance)
{
	...
	inst = (file_instance *) rad_malloc(sizeof *inst);
	if (!inst) {
		return -1;
	}
		       ...
		       VALUE_PAIR *request_pairs, VALUE_PAIR **reply_pairs)
{
	...
	my_pl.name = name;
	user_pl = (PAIR_LIST *)fr_hash_table_finddata(ht, &my_pl);
	my_pl.name = "DEFAULT";
	default_pl = (PAIR_LIST *)fr_hash_table_finddata(ht, &my_pl);

...
{
	rlm_acct_unique_list_t 	*obj;

	obj = (rlm_acct_unique_list_t 	*)rad_malloc(sizeof(*obj));
	memset(obj, 0, sizeof(*obj));

	/* Assign the attr to our obj structure */
	obj->dattr = dattr;
...
{
	rlm_acct_unique_t *inst = (rlm_acct_unique_t *)instance;
	rlm_acct_unique_list_t *self, *next;

	for (self = inst->head; self != NULL; self = next) {
		next = self->next;
		free(self);
	}
	free(inst);
...
static int unique_instantiate(CONF_SECTION *conf, void **instance)
{
	 ...
	 */
	inst = (rlm_acct_unique_t *)rad_malloc(sizeof(*inst));

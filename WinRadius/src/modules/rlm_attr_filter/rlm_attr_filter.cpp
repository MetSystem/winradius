                      ...
                      int *pass, int *fail)
{
	...
	if (check_item->op_token == T_OP_SET) return;
...
{
	struct attr_filter_instance *inst = (attr_filter_instance *)instance;
...
static int attr_filter_instantiate(CONF_SECTION *conf, void **instance)
{
	...
	inst = (attr_filter_instance *)rad_malloc(sizeof *inst);
	if (!inst) {
		return -1;
	}
			      ...
			      RADIUS_PACKET *packet)
{
	...
	for (pl = inst->attrs; pl; pl = pl->next) {
		...
		for (check_item = pl->check;
			...
			/*
			 *    If it is a SET op_token, add the attribute to
		...
		for (vp = *input; vp != NULL; vp = vp->next ) {
			...
			for (check_item = pl->check;
				...
				if ((check_item->attribute == PW_VENDOR_SPECIFIC) &&
				    ...
				    (check_item->op_token == T_OP_CMP_TRUE)) {
					pass++;
					continue;
				}

 ...
 *   Add an extra attribute in each entry <char extra> signifying if we need to delete_flag this
...
static int ippool_instantiate(CONF_SECTION *conf, void **instance)
{
	 ...
	 */
	data = (rlm_ippool_t *)rad_malloc(sizeof(*data));
	if (!data) {
		return -1;
	}
...
static int ippool_accounting(void *instance, REQUEST *request)
{
	...
	switch(acctstatustype){
		case PW_STATUS_STOP:
			...
			fr_MD5Update(&md5_context, (const uint8_t *)xlat_str, strlen(xlat_str));
		...
		while(key_datum.dptr){
			...
			if (data_datum.dptr){
				...
				(data->max_timeout && request->timestamp >= (entry.timestamp + data->max_timeout))))){
					 ...
					 * or if we don't find an entry then delete_flag the session entry so
					 * that we can change the key
					 * Else we don't delete_flag the session entry since we haven't yet deallocated the
		...
		if (found && !mppp){
			...
			if (delete_flag){
				/*
			 	 ...
			 	 * All is well. We delete_flag one entry and we add one entry
		 	 	 */
				 ...
				 * We will also delete_flag that entry on the accounting section so that we only

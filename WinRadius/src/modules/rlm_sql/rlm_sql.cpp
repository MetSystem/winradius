		    ...
		    UNUSED RADIUS_ESCAPE_STRING func)
{
	SQLSOCK *sqlsocket;
	SQL_ROW row;
	SQL_INST *inst = (SQL_INST *)instance;
...
static int generate_sql_clients(SQL_INST *inst)
{
	 ...
	 */
		...
		c = (RADCLIENT *)rad_malloc(sizeof(*c));
		memset(c, 0, sizeof(*c));
...
static int sql_get_grouplist (SQL_INST *inst, SQLSOCK *sqlsocket, REQUEST *request, SQL_GROUPLIST **group_list)
{
	...
	while (rlm_sql_fetch_row(sqlsocket, inst) == 0) {
		...
		if (*group_list == NULL) {
			*group_list = (SQL_GROUPLIST *)rad_malloc(sizeof(SQL_GROUPLIST));
			group_list_tmp = *group_list;
		} else {
			rad_assert(group_list_tmp != NULL);
			group_list_tmp->next =(sql_grouplist *) rad_malloc(sizeof(SQL_GROUPLIST));
...
static int rlm_sql_instantiate(CONF_SECTION * conf, void **instance)
{
	...
	inst = (SQL_INST *) rad_malloc(sizeof(SQL_INST));
	memset(inst, 0, sizeof(SQL_INST));

	inst->config = (SQL_CONFIG *)rad_malloc(sizeof(SQL_CONFIG));
	...
	if (xlat_name == NULL) {
		 ...
		 */
		group_name = (char*) rad_malloc((strlen(xlat_name) + 1 + 11) * sizeof(char));

...
const char *radiusd_version="1.2.1";
...
static void debug_pair_list(UNUSED VALUE_PAIR *vp)
{
	return;
}
...
static int radlog(int lvl, const char *msg, ...)
{
	va_list ap;
	int r;
...
static int log_debug(const char *msg, ...)
{
	va_list ap;
	int r;
...
static void radlog_request(UNUSED int lvl, UNUSED int priority,
						   ...
						   RADIUS_PACKET *resp)
{
	...
	*/
	...
	state = (enum eapsim_clientstates)statevp->vp_integer;
	...
	*/
	...
	subtype = (enum eapsim_subtype)vp->vp_integer;
...
#else
			newstate = (enum eapsim_clientstates)process_eap_sim_start(req, resp);	
#endif
			break;
...
#else
				newstate = (enum eapsim_clientstates) process_eap_sim_challenge(req, resp);	
#endif
				break;
...
{	
	...
	*/
	...
	if(eapsim_checkmac(req->vps, eapaka_mk.K_aut,
		...
		int64_t lrealsqn = sqn[5]*pow(2.0,0) + sqn[4]*pow(2.0,8) + sqn[3]*pow(2.0,16) + 
			sqn[2]*pow(2.0,24) + sqn[1]*pow(2.0,32) + sqn[0]*pow(2.0,40);
		int64_t lreqsqn = reqsqn[5]*pow(2.0,0) + reqsqn[4]*pow(2.0,8) + reqsqn[3]*pow(2.0,16) + 
						   ...
						   RADIUS_PACKET *resp)
{
	...
	*/
	...
	subtype = (enum eapaka_subtype) vp->lvalue;
...
static void map_eap_types(RADIUS_PACKET *req)
{
	...
	* EAP-message with the given type.
		*/
		...
		ep.type.data = (unsigned char *)malloc(vp->length);

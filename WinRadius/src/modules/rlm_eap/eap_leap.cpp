...
LEAP_PACKET *eapleap_alloc(void)
{
	...
	if ((rp = (LEAP_PACKET   *)malloc(sizeof(LEAP_PACKET))) == NULL) {
...
LEAP_PACKET *eapleap_extract(EAP_DS *eap_ds)
{
	...
	packet->challenge = (uint8_t*)malloc(packet->count);
	...
	if (name_len > 0) {
		packet->name =(char*) malloc(name_len + 1);
		if (!packet->name) {
			radlog(L_ERR, "rlm_eap_leap: out of memory");
			eapleap_free(&packet);
			return NULL;
			    ...
			    leap_session_t *session, VALUE_PAIR **reply_vps)
{
	...
	reply->challenge = (unsigned char *)malloc(reply->count);
	 ...
	 */
	reply->name = (char*)malloc(user_name->length + 1);
	if (reply->name == NULL) {
		radlog(L_ERR, "rlm_eap_leap: out of memory");
		eapleap_free(&reply);
		return NULL;
...
int eapleap_compose(EAP_DS *eap_ds, LEAP_PACKET *reply)
{
	...
	case PW_EAP_RESPONSE:
		...
		eap_ds->request->type.data = (unsigned char *)malloc(reply->length);

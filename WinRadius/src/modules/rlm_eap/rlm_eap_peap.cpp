...
static int eappeap_attach(CONF_SECTION *cs, void **instance)
{
	...
	inst = (rlm_eap_peap_t *)malloc(sizeof(*inst));
	if (!inst) {
...
static peap_tunnel_t *peap_alloc(rlm_eap_peap_t *inst)
{
	...
	t = (peap_tunnel_t *)rad_malloc(sizeof(*t));
	memset(t, 0, sizeof(*t));
...
static int eappeap_authenticate(void *arg, EAP_HANDLER *handler)
{
	...
	peap_tunnel_t *peap = (peap_tunnel_t *) tls_session->opaque;
	...
	if (!tls_session->opaque) {
		tls_session->opaque =( peap_tunnel_t * ) peap_alloc(inst);
		peap = ( peap_tunnel_t * ) tls_session->opaque ;

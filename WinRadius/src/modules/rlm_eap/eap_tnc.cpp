...
TNC_PACKET *eaptnc_alloc(void)
{
	...
	if ((rp = (TNC_PACKET   *)malloc(sizeof(TNC_PACKET))) == NULL) {
...
//        memcpy(&packet->flags_ver[1], &data->flags_ver[1], 4);
	 ...
	 */
	packet->data =(uint8_t *) malloc(thisDataLength);
	if (packet->data == NULL) {
		radlog(L_ERR, "rlm_eap_tnc: out of memory");
		eaptnc_free(&packet);
		return NULL;
...
int eaptnc_compose(EAP_DS *eap_ds, TNC_PACKET *reply)
{
	...
	if (reply->code < 3) {
		...
		eap_ds->request->type.data = (uint8_t *)calloc(reply->length, sizeof(unsigned char*));
